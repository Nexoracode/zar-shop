...
// the parts it is made of (search box, cart button…). Only what differs from "everything shown" is stored.
...
const displayParts: Record<string, Record<PageBuilderIndustry, DisplayPart[]>> = {
  HEADER: {
    GENERAL: [
      { id: "logo", label: "لوگو" },
...
export function sectionDisplayParts(sectionId: string, industry: PageBuilderIndustry): DisplayPart[] | null {
  return displayParts[sectionId]?.[industry] ?? null;
}
...
export function pageDisplaySchema(industry: PageBuilderIndustry) {
  return z.record(z.string().min(1).max(80), sectionDisplaySchema).superRefine((display, context) => {
    for (const [sectionId, entry] of Object.entries(display)) {
      const parts = sectionDisplayParts(sectionId, industry);
      if (!parts) {
        context.addIssue({ code: "custom", message: "تنظیمات نمایش برای این بخش تعریف نشده است.", path: [sectionId] });
        continue;
      }
      if (entry.hiddenParts.some((part) => !parts.some((known) => known.id === part))) {
