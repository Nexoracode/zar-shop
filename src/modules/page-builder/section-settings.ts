...
export type PageSectionSettingsBundle = PageSectionSettings & { productLists: Record<string, ProductListConfig>; /** The banner sets added from the page builder (sliders and tile looks), by section id. */ bannerSliders: Record<string, BannerSliderConfig>; /** The category strips added from the page builder, by section id. */ categoryStrips: Record<string, CategoriesSectionSettings>; /** Added sections that are still drafts: not part of the page until a layout containing them is saved. */ draftSectionIds: string[] };
...
export type ContentField = { /** Hide the field while this says so (e.g. the category picker unless the source is a category). */ visibleWhen?: (values: Record<string, string>) => boolean } & (
  ...
  | { name: string; kind: "image"; label: string; hint?: string }
