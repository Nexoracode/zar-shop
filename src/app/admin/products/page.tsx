    ...
    <>
      ...
      <AdminPanel>
        ...
        {products.length ? <>
          ...
          <Table className="hidden md:block"><TableScrollContainer><TableContent aria-label="فهرست محصولات" className="w-full min-w-[860px]"><TableHeader>{["ردیف", "محصول", "کد کالا", "وزن", "موجودی", "وضعیت", "عملیات"].map((head, index) => <TableColumn id={head} key={head} isRowHeader={index === 1} className="bg-white px-5 py-4 text-right text-xs font-bold text-slate-500">{head}</TableColumn>)}</TableHeader><TableBody>{products.map((product: ProductRow, index) => <ProductTableRow key={product.id} product={product} rowNumber={pagination.skip + index + 1} />)}</TableBody></TableContent></TableScrollContainer></Table>
