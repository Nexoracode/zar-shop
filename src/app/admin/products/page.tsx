  ...
  return (
    <>
      ...
      <div className="mb-5 grid grid-cols-3 gap-3">
        {[{ label: "همه محصولات", value: total }, { label: "منتشرشده", value: active }, { label: "پیش‌نویس", value: drafts }].map((item) => <Card key={item.label} variant="secondary" className="rounded-2xl border border-slate-200/80 bg-white p-3.5 sm:p-4"><strong className="block text-xl font-bold text-[var(--accent)]">{item.value.toLocaleString("fa-IR")}</strong><span className="text-[11px] text-slate-500 sm:text-xs">{item.label}</span></Card>)}
      </div>

      <AdminPanel>
        <div className="border-b border-slate-100 bg-slate-50/70 p-4"><AdminListFilters path="/admin/products" query={query} queryLabel="جستجوی محصول" queryPlaceholder="نام، کد کالا یا نشانی محصول" filters={[{ name: "status", label: "وضعیت محصول", value: status ?? "", options: [{ value: "", label: "همه وضعیت‌ها" }, ...Object.entries(productStatusLabels).map(([value, label]) => ({ value, label }))] }, { name: "category", label: "دسته‌بندی", value: params.category ?? "", options: [{ value: "", label: "همه دسته‌ها" }, ...categories.map((category) => ({ value: category.id, label: category.name }))] }, { name: "featured", label: "نمایش ویژه", value: featured ?? "", options: [{ value: "", label: "همه محصولات" }, { value: "yes", label: "محصولات ویژه" }, { value: "no", label: "محصولات عادی" }] }]} /></div>
