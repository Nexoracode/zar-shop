...
export default async function ManualSmsPage() {
  ...
  const [campaigns, initialHiddenColumns] = await Promise.all([
    db.smsCampaign.findMany({ where: { actorId: { not: null } }, orderBy: { createdAt: "desc" }, take: 50 }),
    readHiddenColumns("smsCampaigns"),
  ]);
  ...
  return <>
    ...
    <BlueprintSmsCampaignList key={items.map((item) => `${item.id}:${item.status}:${item.successfulCount}:${item.failedCount}`).join("|")} items={items} initialHiddenColumns={initialHiddenColumns} />
  </>;
}
