...
import { BlueprintHomepageMenuSettings } from "@/components/admin/blueprint/homepage-menu-settings";
...
export default async function HomepageMenuSettingsPage() {
  ...
  const [settings, linkOptions, brandSettings] = await Promise.all([getHomepageSettings(), getHomepageMenuLinkOptions(), getBrandSettings()]);
  return <>
    ...
    {brandSettings.adminTemplate === "BLUEPRINT" ? <BlueprintHomepageMenuSettings initialSettings={settings} linkOptions={linkOptions} /> : <HomepageMenuSettings initialSettings={settings} linkOptions={linkOptions} />}
  </>;
}
