...
export default async function ManualSmsPage() {
  ...
  const [campaigns, brandSettings] = await Promise.all([
  ...
  return <>
    ...
    {brandSettings.adminTemplate === "BLUEPRINT" ? (
      ...
      <SmsCampaignList key={items.map((item) => `${item.id}:${item.status}:${item.successfulCount}:${item.failedCount}`).join("|")} items={items} />
    )}
  </>;
}
