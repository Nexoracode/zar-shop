...
function AuditMetadataPanel({ metadata }: { metadata: unknown }) {
  ...
  return <AdminPanel className="p-5 sm:p-6">
    ...
    {subject && <div className="mb-5 rounded-xl border border-slate-200 p-4"><div className="mb-3 flex items-center gap-2 text-sm font-black text-[#17233b]"><PackageSearch size={17} className="text-[#b5904c]" />آیتم هدف</div><dl className="grid gap-3 sm:grid-cols-2"><DetailItem label="عنوان" value={displayValue(subject.name)} /><DetailItem label="شناسه آیتم" value={displayValue(subject.id)} ltr />{"sku" in subject && <DetailItem label="کد محصول" value={displayValue(subject.sku)} ltr />}{"slug" in subject && <DetailItem label="نشانی" value={displayValue(subject.slug)} ltr />}<DetailItem label="نوع آیتم" value={displayValue(subject.type)} ltr /></dl></div>}
