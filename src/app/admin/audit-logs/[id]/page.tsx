...
function AuditMetadataPanel({ metadata }: { metadata: unknown }) {
  ...
  return <AdminPanel className="p-5 sm:p-6">
    <div className="mb-5 flex items-center gap-2"><Fingerprint size={18} className="text-[var(--warning)]" /><h2 className="m-0 text-base font-bold text-[#17233b]">شرح کامل فعالیت</h2></div>
    {typeof details.summary === "string" && <p className="mb-5 rounded-xl border border-amber-100 bg-amber-50/60 p-4 text-sm font-bold leading-7 text-[var(--warning)]">{details.summary}</p>}
    {subject && <div className="mb-5 rounded-xl border border-slate-200 p-4"><div className="mb-3 flex items-center gap-2 text-sm font-bold text-[#17233b]"><PackageSearch size={17} className="text-[var(--warning)]" />آیتم هدف</div><dl className="grid gap-3 sm:grid-cols-2"><DetailItem label="عنوان" value={displayValue(subject.name)} /><DetailItem label="شناسه آیتم" value={displayValue(subject.id)} ltr />{"sku" in subject && <DetailItem label="کد محصول" value={displayValue(subject.sku)} ltr />}{"slug" in subject && <DetailItem label="نشانی" value={displayValue(subject.slug)} ltr />}<DetailItem label="نوع آیتم" value={displayValue(subject.type)} ltr /></dl></div>}
    {changes.length > 0 && <div><div className="mb-3 flex items-center justify-between gap-3"><span className="flex items-center gap-2 text-sm font-bold text-[#17233b]"><GitCompareArrows size={17} className="text-[var(--warning)]" />تغییرات دقیق</span><span className="rounded-full bg-slate-100 px-3 py-1 text-[11px] font-bold text-slate-500">{changes.length.toLocaleString("fa-IR")} تغییر</span></div><div className="grid gap-3">{changes.map((change, index) => <ChangeCard key={`${change.path}-${index}`} change={change} />)}</div></div>}
