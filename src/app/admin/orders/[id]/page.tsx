      ...
      <div className="grid items-stretch gap-5 lg:grid-cols-2 xl:grid-cols-12">
        ...
        <aside className="contents">
          ...
          <AdminPanel className="order-2 p-5 xl:col-span-4">
            ...
            <dl className="space-y-3 text-sm"><div className="flex justify-between gap-3 text-slate-500"><dt>جمع کالاها</dt><dd>{formatMoney(order.subtotal.toString())}</dd></div>{Number(order.productDiscount) > 0 && <div className="flex justify-between gap-3 text-slate-500"><dt>تخفیف محصولات</dt><dd>{formatMoney(order.productDiscount.toString())}</dd></div>}{Number(order.promotionDiscount) > 0 && <div className="flex justify-between gap-3 text-violet-600"><dt>تخفیف پروموشن</dt><dd>{formatMoney(order.promotionDiscount.toString())}</dd></div>}{Number(order.shippingDiscount) > 0 && <div className="flex justify-between gap-3 text-sky-600"><dt>تخفیف ارسال</dt><dd>{formatMoney(order.shippingDiscount.toString())}</dd></div>}<div className="flex justify-between gap-3 text-slate-500"><dt>هزینه ارسال</dt><dd>{formatMoney(order.shipping.toString())}</dd></div><div className="flex justify-between gap-3 text-slate-500"><dt>مالیات</dt><dd>{formatMoney(order.tax.toString())}</dd></div><div className="flex justify-between gap-3 border-t border-slate-100 pt-3 font-black text-[#17233b]"><dt>مبلغ نهایی</dt><dd>{formatMoney(order.total.toString())}</dd></div>{order.promotionRedemptions.map((redemption) => <div key={redemption.id} className="rounded-lg bg-violet-50 px-3 py-2 text-xs text-violet-700"><dt className="font-bold">{redemption.promotion.title}</dt><dd className="mt-1">{redemption.promotion.code ? `کد: ${redemption.promotion.code}` : "اعمال خودکار"}</dd></div>)}<div className="flex justify-between gap-3 rounded-xl bg-[#f8f1e4] px-3 py-2 text-xs font-bold text-[#846325]"><dt>نرخ طلای ثبت‌شده</dt><dd>{formatMoney(order.goldPriceSnapshot.toString())}</dd></div></dl>
          </AdminPanel>

          {order.invoice && <AdminPanel className="order-3 flex h-full flex-col p-5 xl:col-span-4"><h2 className="mb-3 mt-0 flex items-center gap-2 text-base font-black text-[#17233b]"><FileText size={18} className="text-[#9a7434]" /> فاکتور رسمی</h2><p className="mb-5 mt-0 text-xs text-slate-500">شماره {order.invoice.invoiceNumber} · صادرشده در {formatDateTime(order.invoice.issuedAt)}</p><Link href={`/invoices/${order.id}`} className="mt-auto inline-flex min-h-10 w-full items-center justify-center rounded-xl bg-[#172b4d] px-4 text-sm font-bold text-white transition hover:bg-[#203b66]">مشاهده فاکتور</Link></AdminPanel>}

          {order.notes && <AdminPanel className="order-4 p-5 lg:col-span-2 xl:col-span-12"><h2 className="mb-2 mt-0 text-base font-black text-[#17233b]">یادداشت سفارش</h2><p className="m-0 whitespace-pre-wrap text-sm leading-7 text-slate-600">{order.notes}</p></AdminPanel>}
        </aside>
      </div>
    </>
  );
