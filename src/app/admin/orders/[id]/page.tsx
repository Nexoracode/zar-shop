    ...
    <>
      ...
      <div className="grid items-stretch gap-5 lg:grid-cols-2 xl:grid-cols-12">
        ...
        <aside className="contents">
          ...
          {order.invoice && <AdminPanel className="order-3 flex h-full flex-col p-5 xl:col-span-4"><h2 className="mb-3 mt-0 flex items-center gap-2 text-base font-bold text-[#17233b]"><FileText size={18} className="text-[#9a7434]" /> فاکتور رسمی</h2><p className="mb-5 mt-0 text-xs text-slate-500">شماره {order.invoice.invoiceNumber} · صادرشده در {formatDateTime(order.invoice.issuedAt)}</p><Link href={`/invoices/${order.id}`} className="mt-auto inline-flex min-h-10 w-full items-center justify-center rounded-xl bg-[#172b4d] px-4 text-sm font-bold text-white transition hover:bg-[#203b66]">مشاهده فاکتور</Link></AdminPanel>}
