...
export default async function Home() {
  ...
  return <main className="flex flex-col overflow-hidden bg-[#f7f4f2] pb-[66px] lg:pb-0">
    ...
    <section {...sectionProps("CONCIERGE")} className="bg-white py-[60px]">
      ...
      {activeFaqs.length > 0 && <div id="faq" className={`${container} mt-[90px] overflow-hidden rounded-[7px] border border-[#e5e5e5] bg-[#f7f7f6] lg:grid lg:grid-cols-[340px_1fr]`}><div className="relative overflow-hidden bg-[var(--brand-primary)] p-7 text-[var(--brand-primary-foreground)] lg:p-10"><span className="absolute -bottom-10 -left-4 text-[9rem] font-black leading-none opacity-[0.06]">FAQ</span><span className="relative text-xs font-bold opacity-75">راهنمای خرید</span><h2 className="relative mb-4 mt-2 text-3xl font-black">سوالات متداول</h2><p className="relative m-0 text-sm leading-8 opacity-80">پاسخ روشن به پرسش‌های پرتکرار درباره انتخاب محصول، سفارش، پرداخت و تحویل.</p><span className="relative mt-8 inline-flex rounded-[4px] bg-white/10 px-3 py-2 text-xs font-bold">{activeFaqs.length.toLocaleString("fa-IR")} پاسخ کاربردی</span></div><div className="p-4 sm:p-6 lg:p-8"><StorefrontFaqAccordion faqs={activeFaqs} /></div></div>}
