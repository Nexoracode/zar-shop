...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(await startPendingOrderPayment({ orderId: id, userId: user.id, paymentProvider: parsed.data.paymentProvider, origin: getRequestOrigin(request) }));
  } catch (error) {
