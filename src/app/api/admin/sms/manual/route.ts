...
export async function DELETE(request: Request) {
  try {
    ...
    await db.$transaction(async (tx) => {
      ...
      await tx.auditLog.create({ data: { actorId: actor.id, action: "MANUAL_SMS_DELETE", entityType: "SmsCampaign", entityId: id, metadata: { audience: campaign.audience, recipientCount: campaign.recipientCount, status: campaign.status } } });
    });
