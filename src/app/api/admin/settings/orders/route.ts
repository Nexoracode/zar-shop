...
export async function PATCH(request: Request) {
  try {
    ...
    await db.$transaction(async (transaction) => {
      ...
      await transaction.auditLog.create({ data: { actorId: actor.id, action: "ORDER_SETTINGS_UPDATE", entityType: "StoreSetting", entityId: STORE_SETTING_ID, ...auditRequestContext(request, input) } });
    });
    return NextResponse.json(await getOrderSettings());
  } catch (error) {
