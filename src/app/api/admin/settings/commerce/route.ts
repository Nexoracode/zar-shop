...
export async function PATCH(request: Request) {
  try {
    ...
    await db.$transaction(async (transaction) => {
      ...
      await transaction.auditLog.create({ data: { actorId: actor.id, action: "COMMERCE_SETTINGS_UPDATE", entityType: "StoreSetting", entityId: STORE_SETTING_ID, metadata: input } });
    });
    return NextResponse.json(await getCommerceSettings());
