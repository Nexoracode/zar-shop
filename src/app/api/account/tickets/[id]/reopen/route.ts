...
export async function POST(request: Request, context: Context) {
  try {
    ...
    const ticket = await db.$transaction(async (tx) => {
      const updated = await reopenTicket(tx, { ticketId: id, actorId: user.id, isAgent: false });
      await tx.auditLog.create({ data: { actorId: user.id, action: "TICKET_STATUS_UPDATE", entityType: "SupportTicket", entityId: id, ...auditRequestContext(request, { status: "OPEN" }) } });
      return updated;
    });
