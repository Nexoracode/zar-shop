...
export async function POST(request: Request) {
  try {
    ...
    if (files.some((file) => !EXTENSIONS[file.type] || file.size > MAX_SIZE || (scope === "CATEGORY" && !["image/jpeg", "image/png", "image/webp"].includes(file.type)) || (scope === "HOMEPAGE" && !file.type.startsWith("image/")))) {
