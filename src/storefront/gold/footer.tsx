...
export async function GoldFooter({ settings, brand }: { settings: GeneralStoreSettingsInput; brand: BrandSettings }) {
  ...
  return <footer className="border-t border-[#e7e1da] bg-[#f7f4f2] pb-[66px] pt-12 text-[#4b4a47] lg:pb-0 lg:pt-[60px]">
    <div className="mx-auto grid w-[min(1440px,calc(100%-32px))] gap-10 sm:grid-cols-2 lg:w-[min(1440px,calc(100%-80px))] lg:grid-cols-[1.4fr_repeat(3,1fr)]">
      <div>
      ...
      <div className="flex flex-col items-start gap-3 text-xs"><h4 className="mb-2 mt-0 text-sm font-bold text-[#202020]">{settings.storeName}</h4>{publishedPages.filter((page) => page.id === "ABOUT" || page.id === "CONTACT").map((page) => <Link key={page.id} href={pageHref(page.id)}>{page.title}</Link>)}<Link href="/products">محصولات</Link><Link href="/blog">وبلاگ</Link><Link href="/#trust">مشتریان ما</Link></div>
      <div className="flex flex-col items-start gap-3 text-xs"><h4 className="mb-2 mt-0 text-sm font-bold text-[#202020]">راهنما و قوانین</h4>{publishedPages.filter((page) => page.id !== "ABOUT" && page.id !== "CONTACT").map((page) => <Link key={page.id} href={pageHref(page.id)}>{page.title}</Link>)}<Link href="/account">پیگیری سفارش</Link><Link href="/pages/faq">سوالات متداول</Link></div>
      <div className="flex flex-col items-start gap-3 text-xs"><h4 className="mb-2 mt-0 text-sm font-bold text-[#202020]">با ما در ارتباط باشید</h4>{settings.supportPhone && <a href={phoneHref(settings.supportPhone)} className="flex items-center gap-2"><Phone size={15} />{settings.supportPhone}</a>}{settings.supportEmail && <a href={`mailto:${settings.supportEmail}`} className="flex items-center gap-2" dir="ltr"><Mail size={15} />{settings.supportEmail}</a>}{settings.storeAddress && <span className="flex items-start gap-2"><MapPin className="mt-1 shrink-0" size={15} />{settings.storeAddress}</span>}{settings.supportHours && <span>{settings.supportHours}</span>}</div>
    </div>
    <div className="mx-auto mt-12 w-[min(1440px,calc(100%-32px))] border-t border-[#ddd5cc] py-5 text-center text-[0.68rem] text-[#777] lg:w-[min(1440px,calc(100%-80px))]">کلیه حقوق این وب‌سایت محفوظ و متعلق به {settings.storeName} است.</div>
  </footer>;
}
