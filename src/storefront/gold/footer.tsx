...
export async function GoldFooter({ settings, brand, display, editable }: { settings: GeneralStoreSettingsInput; brand: BrandSettings; display: PageDisplay; /** The viewer can edit the page: a switched-off footer is still rendered so the builder can bring it back. */ editable: boolean }) {
  ...
  return <footer {...builderSectionProps("FOOTER")} className="border-t border-[#e7e1da] bg-[#f7f4f2] pb-[66px] pt-12 text-[#4b4a47] lg:pb-0 lg:pt-[60px]">
    ...
    <div className="mx-auto mt-12 w-[min(var(--store-max-width),calc(100%-32px))] border-t border-[#ddd5cc] py-5 text-center text-[0.68rem] text-[#777] lg:w-[min(var(--store-max-width),calc(100%-80px))]">کلیه حقوق این وب‌سایت محفوظ و متعلق به {settings.storeName} است.</div>
  </footer>;
}
