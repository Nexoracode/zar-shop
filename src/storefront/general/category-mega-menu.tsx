...
export function GeneralCategoryMegaMenu({ categories, enabled = true }: { categories: MenuCategory[]; enabled?: boolean }) {
  ...
  >
    ...
    <div className={`absolute inset-x-0 top-full z-50 border-t border-slate-200 bg-white shadow-[0_18px_38px_rgba(24,35,55,.14)] transition duration-200 ${open && enabled ? "visible pointer-events-auto translate-y-0 opacity-100" : "invisible pointer-events-none translate-y-1 opacity-0"}`} dir="rtl" aria-hidden={!open || !enabled}>
      {activeCategory ? <div className="flex h-[min(620px,calc(100dvh-130px))] min-h-[360px] w-full overflow-hidden">
        <aside className="storefront-mega-scroll w-60 shrink-0 overflow-y-auto overscroll-contain border-l border-slate-200 bg-slate-50" aria-label="دسته‌های اصلی">
          {categories.map((category) => {
            ...
            return <Link key={category.id} href={`/products?category=${category.slug}`} onMouseEnter={() => setActiveCategoryId(category.id)} onFocus={() => setActiveCategoryId(category.id)} onClick={() => setOpen(false)} className={`flex min-h-14 items-center gap-3 border-r-2 px-4 text-xs font-bold transition ${active ? "border-[var(--brand-primary)] bg-white text-[var(--brand-primary)]" : "border-transparent text-slate-700 hover:bg-white hover:text-[var(--brand-primary)]"}`}>
              <span className={`relative grid size-8 shrink-0 place-items-center overflow-hidden rounded-lg ${active ? "bg-[var(--brand-primary)]/10" : "bg-white"}`}>{category.image?.type === "IMAGE" ? <Image src={category.image.url} alt={category.image.alt ?? category.name} fill sizes="32px" className="object-cover" /> : <Package size={17} />}</span>
              <span className="min-w-0 flex-1 truncate">{category.name}</span><ChevronLeft size={14} className="shrink-0" />
            </Link>;
          })}
        </aside>
      ...
      </div> : <div className="px-6 py-12 text-center text-xs text-slate-500">دسته‌بندی فعالی برای نمایش در منوی فروشگاه انتخاب نشده است.</div>}
    </div>
  </div>;
}
