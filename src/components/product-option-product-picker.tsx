      ...
      <section className="min-w-0">
        {loadingOptions ? <LoadingOptionsPanel productName={selectedProduct?.name ?? "محصول"} /> : optionError && selectedProduct ? <OptionsErrorPanel message={optionError} onRetry={() => void selectProduct(selectedProduct)} /> : optionData && selectedProduct ? (
          <div className="grid gap-4">
            <Card variant="secondary" className="rounded-2xl border border-violet-200 bg-violet-50/50 shadow-sm"><Card.Content className="flex flex-wrap items-center justify-between gap-3 p-4"><div><span className="text-[10px] font-bold text-violet-500">محصول انتخاب‌شده</span><strong className="mt-1 block text-sm text-slate-800">{optionData.productName}</strong><span dir="ltr" className="mt-0.5 block text-right text-[11px] text-slate-500">{optionData.productSku}</span></div><AdminStatusBadge tone={productStatusTones[selectedProduct.status]}>{productStatusLabels[selectedProduct.status]}</AdminStatusBadge></Card.Content></Card>
