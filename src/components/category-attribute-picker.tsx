        ...
        <Card variant="secondary" className="overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm lg:h-[min(620px,calc(100dvh-128px))] lg:min-h-[460px]">
          <Card.Content className="flex h-full min-h-0 flex-col p-4">
            <div className="mb-4 flex shrink-0 items-start gap-3 border-b border-slate-100 pb-4">
              ...
              <div><h2 className="m-0 text-sm font-bold text-slate-800">انتخاب دسته‌بندی</h2><p className="mt-1 text-[11px] leading-5 text-slate-500">نام، نشانی یا دسته والد را بنویسید.</p></div>
            </div>
...
function DisabledPanel() {
  return <Card variant="secondary" className="min-h-[420px] rounded-xl border border-slate-200 bg-white opacity-60 shadow-sm" aria-disabled="true"><Card.Content className="grid gap-4 p-4 sm:p-5"><div className="flex items-center justify-between gap-3 border-b border-slate-100 pb-4"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-slate-100 text-slate-400"><Layers3 size={19} /></span><div><h2 className="m-0 text-base font-bold text-slate-700">ساختار ویژگی‌ها</h2><p className="mt-1 text-xs text-slate-400">پس از انتخاب دسته‌بندی فعال می‌شود.</p></div></div><Button type="button" variant="secondary" isDisabled className="gap-2 text-xs"><Plus size={14} />گروه جدید</Button></div><div className="grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4"><Input disabled fullWidth variant="secondary" placeholder="نام گروه؛ مثلاً مشخصات کلی" className={adminFieldClass} /><Input disabled fullWidth variant="secondary" placeholder="نام ویژگی؛ مثلاً رم یا مناسب برای" className={adminFieldClass} /></div><div className="grid min-h-40 place-items-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-4 text-center"><div><LockKeyhole className="mx-auto mb-3 text-slate-300" size={30} /><strong className="block text-sm text-slate-600">ابتدا یک دسته‌بندی انتخاب کنید</strong><span className="mt-1 block text-xs leading-6 text-slate-400">فرم تعریف گروه و ویژگی پس از انتخاب دسته‌بندی در دسترس قرار می‌گیرد.</span></div></div></Card.Content></Card>;
}
