...
export function ProductAttributesForm({ productId, categoryId, categoryName, groups, initialAttributes }: Props) {
  ...
  return <form onSubmit={submit} className="admin-sticky-save-form grid gap-4">
    ...
    <Card variant="secondary" className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><strong className="block text-sm text-slate-800">ذخیره ویژگی‌های محصول</strong><span className="mt-1 hidden text-xs text-slate-500 sm:block">{completedCount.toLocaleString("fa-IR")} ویژگی از {definitions.length.toLocaleString("fa-IR")} ویژگی تکمیل شده است.</span></div><Button type="submit" variant="primary" isPending={saving} className="min-h-11 shrink-0 gap-2 px-5 font-bold">{({ isPending }) => <>{isPending ? <Spinner size="sm" color="current" /> : completedCount === definitions.length ? <CheckCircle2 size={16} /> : <Save size={16} />}{isPending ? "در حال ذخیره..." : "ذخیره ویژگی‌ها"}</>}</Button></div></Card>
  </form>;
}
