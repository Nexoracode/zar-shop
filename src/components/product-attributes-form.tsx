...
export function ProductAttributesForm({ productId, categoryId, categoryName, groups, initialAttributes }: Props) {
  ...
  if (!categoryId) return <Card variant="secondary" className="rounded-2xl border border-[color-mix(in_srgb,var(--warning)_30%,transparent)] bg-[color-mix(in_srgb,var(--warning)_14%,transparent)] p-6 text-center"><ListChecks className="mx-auto mb-3 text-[var(--warning)]" size={30} /><strong className="block text-sm text-[var(--warning)]">محصول دسته‌بندی ندارد</strong><p className="mt-2 text-xs text-[var(--warning)]">ابتدا از فرم محصول یک دسته‌بندی انتخاب و ذخیره کنید.</p><Link href={`/admin/products/${productId}/edit`} className="mt-4 inline-flex min-h-10 items-center justify-center rounded-xl bg-white px-5 text-xs font-bold text-[var(--warning)]">ویرایش محصول</Link></Card>;
