...
export function ProductAttributesForm({ productId, categoryId, categoryName, groups, initialAttributes }: Props) {
  ...
  return <form onSubmit={submit} className="admin-sticky-save-form grid gap-4">
    ...
    <Card variant="secondary" className="rounded-xl border border-slate-200 bg-white p-2.5 shadow-sm"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><strong className="block text-xs font-black text-slate-800">ذخیره ویژگی‌های محصول</strong><span className="mt-1 hidden text-[10px] text-slate-500 sm:block">{completedCount.toLocaleString("fa-IR")} ویژگی از {definitions.length.toLocaleString("fa-IR")} ویژگی تکمیل شده است.</span></div><Button type="submit" size="sm" variant="primary" isPending={saving} isDisabled={saving} aria-label={saving ? "در حال ذخیره ویژگی‌ها" : "ذخیره ویژگی‌ها"} className="!h-9 !min-h-9 shrink-0 gap-1.5 rounded-lg px-3 !text-[10px] !font-normal">{saving ? <><Spinner size="sm" color="current" />در حال ذخیره...</> : <>{completedCount === definitions.length ? <CheckCircle2 className="!size-3.5" /> : <Save className="!size-3.5" />}ذخیره ویژگی‌ها</>}</Button></div></Card>
  </form>;
}
