  ...
  return (
    <div dir="rtl">
      ...
      <div className="relative min-h-[260px]" aria-live="polite" aria-busy={loading}>
        ...
        {error ? <Alert status="danger" className="min-h-[180px] place-content-center text-right"><Alert.Description><span className="block">{error}</span><Button type="button" size="sm" variant="secondary" isPending={loading} onPress={() => void load(feed.sort, feed.pagination.page)} className="mt-3 gap-2"><RotateCw size={14} />تلاش دوباره</Button></Alert.Description></Alert> : feed.items.length ? (
