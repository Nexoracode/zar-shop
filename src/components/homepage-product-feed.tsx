    ...
    <div dir="rtl">
      {industry === "GOLD" && <div className="mb-7 flex items-end justify-between gap-4">
        <nav className="flex flex-wrap gap-2" aria-label="مرتب‌سازی محصولات">
          {filters.filter((filter) => industry === "GOLD" || filter.id !== "LOW_FEE").map((filter) => <Button key={filter.id} id={filter.id === "LATEST" ? "latest-products" : undefined} type="button" size="sm" variant={feed.sort === filter.id ? "primary" : "secondary"} isPending={pendingSort === filter.id} isDisabled={loading} onPress={() => void load(filter.id, 1)} className={`${filter.mobile ? "" : "hidden sm:inline-flex"} min-h-9 rounded-full px-5 text-xs font-bold ${feed.sort === filter.id ? "bg-[var(--brand-primary)] text-[var(--brand-primary-foreground)]" : "bg-[#fafafa] text-[#333]"}`}>{filter.label}</Button>)}
        </nav>
      ...
      <div className="relative min-h-[260px]" aria-live="polite" aria-busy={loading}>
        ...
        {error ? (
          <InlineAlert
            status="danger"
            className="min-h-[180px] items-center"
            action={<Button type="button" size="sm" variant="secondary" isPending={loading} onPress={() => void load(feed.sort, feed.pagination.page)} className="gap-2"><RotateCw size={14} />تلاش دوباره</Button>}
          >
            {error}
          </InlineAlert>
        ) : feed.items.length ? (
