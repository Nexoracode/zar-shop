    ...
    <div dir="rtl">
      ...
      <div className="relative min-h-[260px]" aria-live="polite" aria-busy={loading}>
        ...
        {error ? <Alert status="danger" className="min-h-[180px] place-content-center text-right"><Alert.Description><span className="block">{error}</span><Button type="button" size="sm" variant="secondary" onPress={() => void load(feed.sort, feed.pagination.page)} className="mt-3 gap-2"><RotateCw size={14} />تلاش دوباره</Button></Alert.Description></Alert> : feed.items.length ? (
          <DragScrollRow key={`${feed.sort}-${feed.pagination.page}`} ariaLabel="محصولات فروشگاه" className={`flex w-full min-w-0 max-w-full gap-4 overflow-x-auto pb-4 ${industry === "GENERAL" ? "storefront-product-scroll" : "[scrollbar-width:none] [&::-webkit-scrollbar]:hidden"}`}>
            ...
            {industry === "GENERAL" && <div className="w-[calc(50%-8px)] min-w-[calc(50%-8px)] sm:w-[220px] sm:min-w-[220px] lg:w-[224px] lg:min-w-[224px]"><ViewAllProductCard href="/products" /></div>}
          </DragScrollRow>
