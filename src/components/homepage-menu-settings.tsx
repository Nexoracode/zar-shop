      ...
      <Card.Content className="grid gap-4 p-4">
        <div className="grid gap-3 lg:grid-cols-2">
          ...
          <div className="grid gap-3 rounded-xl border border-dashed border-[var(--border)] p-3 sm:grid-cols-[minmax(120px,.7fr)_minmax(180px,1.3fr)_auto] sm:items-end">
            <MenuField label="عنوان"><Input value={customLabel} onChange={(event) => setCustomLabel(event.target.value)} placeholder="پیشنهاد ویژه" variant="secondary" className={adminFieldClass} /></MenuField>
            <MenuField label="لینک"><Input value={customHref} onChange={(event) => setCustomHref(event.target.value)} dir="ltr" placeholder="/products" variant="secondary" className={adminFieldClass} /></MenuField>
        ...
        {items.length ? <div className="grid gap-2 lg:grid-cols-2">{items.map((item, index) => <Card key={item.id} variant="secondary" onDragOver={(event) => { event.preventDefault(); moveItem(item.id); }} onDrop={(event) => { event.preventDefault(); setDraggedId(null); }} className={`grid gap-2 rounded-xl border p-2.5 transition sm:grid-cols-[auto_minmax(100px,.65fr)_minmax(160px,1.35fr)_auto] sm:items-end ${draggedId === item.id ? "border-[var(--accent)] opacity-55" : "border-[var(--border)]"}`}>
          ...
          <MenuField label={`عنوان ${(index + 1).toLocaleString("fa-IR")}`}><Input value={item.label} onChange={(event) => setItems((current) => current.map((currentItem) => currentItem.id === item.id ? { ...currentItem, label: event.target.value } : currentItem))} variant="secondary" className={adminFieldClass} /></MenuField>
          <MenuField label="لینک"><Input value={item.href} onChange={(event) => setItems((current) => current.map((currentItem) => currentItem.id === item.id ? { ...currentItem, href: event.target.value } : currentItem))} dir="ltr" variant="secondary" className={adminFieldClass} /></MenuField>
