...
/** The check on a selected colour: dark on a light swatch, white on a dark one, so it always reads. */
function readableOn(hex: string | undefined) {
  ...
  return (red * 299 + green * 587 + blue * 114) / 1000 > 150 ? "#1f2937" : "#ffffff";
}
...
export function AddToCart({ productId, options = [], variants = [], optionGuide, disabled, disabledLabel = "ناموجود", currency = "IRR", layout = "default", purchaseSummary, purchaseMeta, purchaseFooter, purchasePrice = null, purchaseOriginalPrice = null, preparationDays = 0, showOptionFields = true, showPurchaseCard = true, showMobileBar = false, purchaseCardClassName, purchaseCardStickyTop = "6rem", detailedCard, storeName }: { productId: string; options?: ProductOption[]; variants?: PurchasableVariant[]; optionGuide?: OptionGuide | null; disabled: boolean; disabledLabel?: string; currency?: "IRR" | "IRT"; layout?: "default" | "product-detail"; purchaseSummary?: ReactNode; purchaseMeta?: ReactNode; /** Rendered under the trust badges, once per card instance — e.g. a "chat with support" link. */ purchaseFooter?: ReactNode; purchasePrice?: number | null; purchaseOriginalPrice?: number | null; preparationDays?: number; showOptionFields?: boolean; showPurchaseCard?: boolean; /** Renders the persistent mobile bottom price+buy bar — set on exactly one of the page's `AddToCart` instances. */ showMobileBar?: boolean; purchaseCardClassName?: string; purchaseCardStickyTop?: string; /** Turns the purchase card into the marketplace-style one: the product itself, the choices made, who sells it, stock, the price and — while a time-limited discount runs — the "پیشنهاد شگفت‌انگیز" header. Only the page's second card asks for it. */ detailedCard?: { name: string; imageUrl: string | null; imageAlt: string; storeName: string }; /** Who sells it — shown under the first purchase card's heading. The detailed card carries its own. */ storeName?: string }) {
  ...
  // The guide (a size chart, usually) is offered in the header of the first option that is not a colour,
  // where the visitor is choosing a size; with only colours it stays at the top of the block.
  const guideOption = options.find((option) => option.kind !== "COLOR") ?? null;
  const guideButton = <Button type="button" variant="ghost" onPress={() => setGuideOpen(true)} className="h-auto min-h-0 gap-1 bg-transparent p-0 text-xs font-bold text-[var(--brand-accent)] hover:bg-transparent! data-[hovered=true]:bg-transparent! data-[pressed=true]:bg-transparent!"><Ruler size={15} />راهنمای انتخاب</Button>;
  ...
  const optionFields = options.length > 0 ? <div className="grid gap-7">
        ...
        {options.map((option) => {
          ...
          return <div key={option.id} className="grid gap-4">{option.kind === "COLOR" ? <><span className="inline-flex items-center gap-2 text-sm text-slate-800"><span>رنگ:</span><strong className="text-slate-900">{selectedValue?.color?.name ?? selectedValue?.value ?? "انتخاب کنید"}</strong>{selectedValue?.color && <span aria-hidden className="size-3.5 rounded-full border border-black/10" style={{ backgroundColor: selectedValue.color.hex }} />}</span><div className="flex flex-wrap gap-2.5 py-1" role="group" aria-label={`انتخاب ${option.name}`}>{option.values.map((item) => { const selected = selectedOptions[option.id] === item.value; const unavailable = item.stock < 1; return <Button key={item.value} type="button" isIconOnly variant="secondary" isDisabled={unavailable} aria-pressed={selected} aria-label={`${item.value}${item.color ? `، رنگ ${item.color.name}` : ""}${unavailable ? "، ناموجود" : selected ? "، انتخاب‌شده" : ""}`} onPress={() => setSelectedOptions((current) => ({ ...current, [option.id]: item.value }))} className={`relative size-10 min-h-10 min-w-10 rounded-full border-[1.5px] bg-white p-[3px] transition-colors ${unavailable ? "border-slate-200 opacity-45" : selected ? "border-[var(--brand-accent)]" : "border-slate-200 hover:border-slate-400"}`}><span className="block size-full rounded-full border border-black/10" style={{ backgroundColor: item.color?.hex ?? "var(--separator)" }} />{selected && <Check size={16} strokeWidth={3} aria-hidden className="absolute inset-0 m-auto" style={{ color: readableOn(item.color?.hex) }} />}{unavailable && <span className="absolute inset-1 rotate-45 border-l border-slate-500" />}</Button>; })}</div></> : <><div className="flex items-center justify-between gap-3"><span className="inline-flex items-center gap-1.5 text-sm text-slate-800"><span>{option.name}:</span><strong className="text-slate-900">{selectedValue?.value ?? "انتخاب کنید"}</strong></span>{optionGuide && guideOption?.id === option.id && guideButton}</div><div className="flex flex-wrap gap-2" role="group" aria-label={`انتخاب ${option.name}`}>{option.values.map((item) => { const selected = selectedOptions[option.id] === item.value; const unavailable = item.stock < 1; return <Button key={item.value} type="button" variant="secondary" isDisabled={unavailable} aria-pressed={selected} aria-label={`${item.value}${unavailable ? "، ناموجود" : selected ? "، انتخاب‌شده" : ""}`} onPress={() => setSelectedOptions((current) => ({ ...current, [option.id]: item.value }))} className={`min-h-10 min-w-11 gap-1.5 rounded-lg border bg-white px-3.5 text-sm font-medium transition-colors ${unavailable ? "border-dashed border-slate-300 bg-slate-100 text-slate-400" : selected ? "border-[var(--brand-accent)] text-[var(--brand-accent)]" : "border-slate-200 text-slate-800 hover:border-slate-400"}`}>{item.value}{selected && !unavailable && <Check size={14} strokeWidth={3} aria-hidden />}</Button>; })}</div></>}</div>;
        })}
      </div> : null;
