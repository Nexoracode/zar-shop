...
type ProductOption = { id: string; name: string; values: Array<{ value: string; stock: number; priceAdjustment: number; color: { name: string; hex: string } | null }> };
  ...
  return (
    <div className="grid gap-3">
      {options.length > 0 && <div className="grid gap-4">
        ...
        {options.map((option) => <div key={option.id} className="grid gap-2"><span className="text-xs font-bold text-[#525966]">{option.name}</span><div className="flex flex-wrap gap-2" role="group" aria-label={`انتخاب ${option.name}`}>{option.values.map((item) => { const selected = selectedOptions[option.id] === item.value; const unavailable = item.stock < 1; return <Button key={item.value} type="button" variant="secondary" isDisabled={unavailable} aria-pressed={selected} aria-label={`${item.value}${item.color ? `، رنگ ${item.color.name}` : ""}${item.priceAdjustment ? `، ${formatMoney(item.priceAdjustment)} افزایش قیمت` : ""}${unavailable ? "، ناموجود" : selected ? "، انتخاب‌شده" : ""}`} onPress={() => setSelectedOptions((current) => ({ ...current, [option.id]: item.value }))} className={`min-h-10 min-w-12 rounded-lg border px-3 text-sm font-bold transition ${unavailable ? "border-slate-200 bg-slate-100 text-slate-400 line-through" : selected ? "border-[#b5904c] bg-[#b5904c] text-white ring-2 ring-[#b5904c]/25" : "border-[#d8d5ce] bg-white text-[#17233b] hover:border-[#b5904c]"}`}>{item.color && <span className="ml-2 inline-block h-5 w-5 rounded-full border-2 border-white shadow ring-1 ring-slate-300" style={{ backgroundColor: item.color.hex }} />}{item.value}{item.priceAdjustment > 0 && <small className="mr-1 text-[10px] opacity-75">+{formatMoney(item.priceAdjustment)}</small>}{unavailable && <small className="mr-1 text-[10px]">ناموجود</small>}</Button>; })}</div></div>)}
      </div>}
      <Button
        onPress={() => void add()}
        isDisabled={disabled || optionStockUnavailable || loading}
        fullWidth
        variant="primary"
      ...
      >
        {disabled ? disabledLabel : optionStockUnavailable ? "تنوع موجودی ندارد" : loading ? "در حال افزودن..." : "افزودن به سبد"}
      </Button>
