...
export function AddToCart({ productId, options = [], optionGuide, disabled, disabledLabel = "ناموجود", currency = "IRR", layout = "default", purchaseSummary, purchaseMeta, purchasePrice = null, purchaseOriginalPrice = null, discountLabel, preparationDays = 0, showOptionFields = true, showPurchaseCard = true, purchaseCardClassName, purchaseCardStickyTop = "6rem" }: { productId: string; options?: ProductOption[]; optionGuide?: OptionGuide | null; disabled: boolean; disabledLabel?: string; currency?: "IRR" | "IRT"; layout?: "default" | "product-detail"; purchaseSummary?: ReactNode; purchaseMeta?: ReactNode; purchasePrice?: number | null; purchaseOriginalPrice?: number | null; discountLabel?: string | null; preparationDays?: number; showOptionFields?: boolean; showPurchaseCard?: boolean; purchaseCardClassName?: string; purchaseCardStickyTop?: string }) {
  ...
  const addButton = addedQuantity > 0 ? <div className="grid gap-2"><div className="flex min-h-12 items-center overflow-hidden rounded-lg border border-[var(--brand-primary)] bg-[var(--surface)]"><Button type="button" isIconOnly variant="ghost" isPending={loading} aria-label="افزودن یک عدد دیگر" onPress={() => void add()} className="size-11 min-h-11 min-w-11 text-[var(--brand-primary)]"><Plus size={17} /></Button><span className="grid min-w-9 flex-1 place-items-center text-sm font-black text-[var(--brand-primary)]">{addedQuantity.toLocaleString("fa-IR")}</span><span className="ml-3 flex items-center gap-1 text-xs font-bold text-emerald-700"><Check size={15} />در سبد شما</span></div><Link href="/cart" className="flex min-h-11 items-center justify-center gap-2 rounded-lg bg-[var(--brand-primary)] px-4 text-sm font-black text-[var(--brand-primary-foreground)]"><ShoppingCart size={17} />مشاهده سبد خرید</Link></div> : primaryAddButton;
  ...
  if (layout === "product-detail") return <>
    ...
    {showPurchaseCard && <aside className={purchaseCardClassName ?? "lg:col-start-3 lg:row-span-2 lg:row-start-1"}>
      <div className="grid gap-4 rounded-xl border border-slate-200/80 bg-slate-50/40 p-4 lg:sticky" style={{ top: purchaseCardStickyTop }}>
        ...
        {purchaseSummary}
        {displayedOriginalPrice !== null && displayedPrice !== null && displayedOriginalPrice > displayedPrice && <div className="flex items-center gap-2"><span className="text-xs text-slate-400 line-through">{formatMoney(displayedOriginalPrice, currency)}</span>{discountLabel && <span className="inline-flex items-center rounded-full bg-[var(--danger)] px-2 py-1 text-[10px] font-black text-[var(--danger-foreground)]">{discountLabel}</span>}</div>}
        <strong className="text-left text-xl font-black text-slate-900" dir="rtl">{displayedPrice === null ? "قیمت نامشخص" : formatMoney(displayedPrice, currency)}</strong>
