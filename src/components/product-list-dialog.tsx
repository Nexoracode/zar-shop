...
export function ProductListDialog({ listId, initial, categoryOptions, onSaved, onClose }: { listId: string; initial: ProductListConfig; categoryOptions: CategoryOption[]; onSaved: (config: ProductListConfig) => void; onClose: () => void }) {
  ...
  return (
    <ContentFormDialog
      ...
      fields={fields}
      initial={{ title: initial.title, layout: initial.layout, source: initial.source, categoryId: initial.categoryId ?? "", limit: String(initial.limit) }}
      schema={productListConfigSchema}
      toInput={(values) => ({ layout: values.layout, title: values.title, source: values.source, categoryId: values.source === "CATEGORY" && values.categoryId ? values.categoryId : null, limit: values.limit === "" ? Number.NaN : Number(values.limit) })}
      save={save}
