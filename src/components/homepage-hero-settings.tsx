      ...
      <Card variant="secondary" className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-sm">
        ...
        <Card.Content className="grid gap-4 p-5">
          ...
          {contentMode === "WITH_CONTENT" ? <>
            <div className="grid gap-4 lg:grid-cols-2"><Field label="عنوان اصلی"><Input required maxLength={homepageFieldLimits.heroTitle} value={title} onChange={(event) => setTitle(event.target.value)} variant="secondary" className={adminFieldClass} /></Field><Field label="متن دکمه"><Input required maxLength={homepageFieldLimits.heroButtonLabel} value={buttonLabel} onChange={(event) => setButtonLabel(event.target.value)} variant="secondary" className={adminFieldClass} /></Field></div>
            <Field label="متن کوتاه"><TextArea required maxLength={homepageFieldLimits.heroDescription} value={description} onChange={(event) => setDescription(event.target.value)} rows={3} variant="secondary" className={adminFieldClass} /></Field>
          </> : null}
        </Card.Content>
      </Card>
      ...
      <Card variant="secondary" className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-sm">
        ...
        <Card.Content className="p-5">
          {slides.length ? <div className="grid gap-4 xl:grid-cols-2">{slides.map((slide, index) => {
            ...
            <div className="mt-3"><Field label="لینک اختصاصی اسلاید"><Input required maxLength={homepageFieldLimits.href} value={slide.href} onChange={(event) => setSlides((current) => current.map((item) => item.id === slide.id ? { ...item, href: event.target.value } : item))} dir="ltr" placeholder="/products یا https://example.com" variant="secondary" className={adminFieldClass} /></Field></div>
          </Card></div>;
