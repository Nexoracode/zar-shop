...
export function AdminSidebar({ user }: Props) {
  ...
  return (
    <>
      <div className="flex items-center justify-between rounded-2xl bg-[#172b4d] p-3 text-white lg:hidden"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#b5904c] font-bold">زر</span><div><strong className="block text-sm">پنل مدیریت</strong><span className="block text-[11px] text-white/55">{fullName}</span></div></div><Button type="button" onPress={() => setOpen(true)} aria-label="باز کردن منوی مدیریت" isIconOnly variant="ghost" className="h-10 w-10 min-w-10 rounded-xl bg-white/10 text-white"><Menu size={20} /></Button></div>
      {open && <div className="fixed inset-0 z-[120] bg-slate-950/55 lg:hidden" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><aside className="ml-auto flex h-full w-[min(84vw,310px)] flex-col bg-[#172b4d] p-4 text-white shadow-2xl"><div className="mb-5 flex items-center justify-between"><strong>منوی مدیریت</strong><Button type="button" onPress={() => setOpen(false)} aria-label="بستن منوی مدیریت" isIconOnly variant="ghost" className="h-10 w-10 min-w-10 rounded-xl bg-white/10 text-white"><X size={19} /></Button></div>{navigation}<div className="mt-auto grid gap-2 border-t border-white/10 pt-4"><Link href="/" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-white/70"><Store size={18} />مشاهده فروشگاه</Link><Button type="button" isPending={loggingOut} onPress={() => void logout()} variant="ghost" className="justify-start gap-3 rounded-xl px-3 py-2.5 text-sm text-rose-200"><LogOut size={18} />خروج از حساب</Button></div></aside></div>}
      <aside className="hidden self-start overflow-hidden rounded-[24px] bg-[#172b4d] p-4 text-white shadow-[0_24px_70px_rgba(16,35,62,0.18)] lg:sticky lg:top-5 lg:flex lg:h-[calc(100dvh-2.5rem)] lg:min-h-0 lg:flex-col">
        <div className="mb-5 flex items-center gap-3 border-b border-white/10 px-1 pb-5"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#b5904c] font-bold shadow-lg">زر</span><div className="min-w-0"><strong className="block text-sm">مدیریت زر گالری</strong><small className="block truncate text-[11px] text-white/50">مرکز عملیات فروشگاه</small></div></div>
        {navigation}
      </aside>
    </>
  );
