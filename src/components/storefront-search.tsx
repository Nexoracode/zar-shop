        ...
        <Modal.Dialog style={panelStyle} aria-label="جستجوی محصولات" dir="rtl" className="m-0 max-w-none origin-top overflow-hidden rounded-[24px] bg-white p-0 shadow-2xl">
          <Modal.Header className="block p-2">
            <form onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); searchAll(); }} className="relative">
              ...
              <Input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} aria-label="عبارت جستجو" placeholder="جستجو در همه کالاها" variant="secondary" className={`h-11 min-h-11 w-full rounded-xl border-0 bg-slate-100 pr-11 text-sm font-normal text-slate-700 outline-none ring-0 placeholder:text-slate-400 ${query ? "pl-11" : "pl-4"}`} />
