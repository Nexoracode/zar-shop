...
function InitialSearchContent({ recent, popularTerms, loading, onClearRecent, onSelect, onClose }: { recent: string[]; popularTerms: string[]; loading: boolean; onClearRecent: () => void; onSelect: (value: string) => void; onClose: () => void }) {
  return <div className="grid gap-6 p-4 sm:p-5">
    ...
    <Link href="/products?sortby=popular" onClick={onClose} className="block"><Card className="relative overflow-hidden rounded-2xl bg-[linear-gradient(120deg,#111827,#334155)] p-5 text-white shadow-sm"><Sparkles className="absolute -left-3 -top-5 size-28 opacity-10" /><span className="relative block text-[11px] text-white/70">پیشنهاد امروز فروشگاه</span><strong className="relative mt-1 block text-lg font-bold">محبوب‌ترین انتخاب‌ها را ببینید</strong><Chip className="relative mt-3 w-fit bg-white text-slate-900"><Chip.Label className="text-xs font-bold">مشاهده محصولات</Chip.Label></Chip></Card></Link>
  </div>;
}
