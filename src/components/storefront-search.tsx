        ...
        <Modal.Dialog style={panelStyle} aria-label="جستجوی محصولات" dir="rtl" className="m-0 max-w-none origin-top overflow-hidden rounded-[24px] bg-white p-0 shadow-2xl">
          <Modal.Header className="block p-2">
            <form onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); searchAll(); }} className="relative">
              ...
              <Input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} aria-label="عبارت جستجو" placeholder="جستجو در همه کالاها" variant="secondary" style={{ fontSize: 17 }} className={`h-11 min-h-11 w-full rounded-xl border-0 bg-slate-100 pr-11 font-normal text-slate-700 outline-none ring-0 placeholder:text-slate-400 ${query ? "pl-11" : "pl-4"}`} />
...
function InitialSearchContent({ recent, popularTerms, loading, onClearRecent, onSelect, onClose }: { recent: string[]; popularTerms: string[]; loading: boolean; onClearRecent: () => void; onSelect: (value: string) => void; onClose: () => void }) {
  return <div className="grid gap-6 p-4 sm:p-5">
    {recent.length > 0 && <section><div className="mb-3 flex items-center justify-between"><h2 className="m-0 text-sm font-black text-slate-800">جستجوهای اخیر</h2><Button type="button" size="sm" variant="ghost" onPress={onClearRecent} className="min-h-8 px-2 text-xs font-normal text-slate-500">پاک کردن</Button></div><div className="flex flex-wrap gap-2">{recent.map((term) => <Button key={term} type="button" size="sm" variant="secondary" onPress={() => onSelect(term)} className="min-h-9 max-w-full rounded-full border border-slate-200 bg-white px-3.5 text-xs font-normal text-slate-700"><Clock3 size={14} /><span className="max-w-[min(360px,70vw)] truncate">{term}</span></Button>)}</div></section>}
    <section><h2 className="mb-3 mt-0 text-sm font-black text-slate-800">جستجوهای پرتکرار</h2>{loading && !popularTerms.length ? <div className="grid min-h-20 place-items-center"><Spinner size="sm" /></div> : <div className="flex flex-wrap gap-2">{popularTerms.map((term, index) => <Button key={term} type="button" size="sm" variant="secondary" onPress={() => onSelect(term)} className="min-h-9 max-w-full rounded-full border border-slate-200 bg-white px-3.5 text-xs font-normal text-slate-700">{index < 3 ? <TrendingUp size={14} /> : <Search size={14} />}<span className="max-w-[min(360px,70vw)] truncate">{term}</span></Button>)}</div>}</section>
