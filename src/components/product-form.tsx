    ...
    <form onSubmit={submit} noValidate className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_380px]">
      <div className="grid gap-5">
        ...
        <FormSection icon={<ListChecks size={18} />} title="ویژگی‌های محصول" description="خلاصه مشخصات ثبت‌شده؛ افزودن و ویرایش در صفحه اختصاصی انجام می‌شود." help={{ summary: "فرم اصلی محصول فقط وضعیت تکمیل ویژگی‌ها را نشان می‌دهد تا صفحه خلوت و سریع باقی بماند.", blocks: [{ title: "مدیریت مستقل", description: "پس از ثبت محصول، از دکمه مدیریت ویژگی‌ها وارد صفحه اختصاصی شوید و مقادیر توصیفی را تکمیل کنید." }, { title: "تغییر دسته‌بندی", tone: "important", description: "اگر دسته‌بندی محصول را تغییر داده‌اید، ابتدا فرم را ذخیره کنید؛ سپس ویژگی‌های متناسب با دسته جدید را وارد کنید." }] }}>
          {!categoryId ? <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-5 py-7 text-center text-xs text-slate-500">برای مشاهده ویژگی‌ها ابتدا دسته‌بندی محصول را انتخاب کنید.</div> : attributeDefinitions.length === 0 ? <div className="rounded-xl border border-[color-mix(in_srgb,var(--warning)_30%,transparent)] bg-[color-mix(in_srgb,var(--warning)_14%,transparent)] p-4"><strong className="block text-sm text-[var(--warning)]">دسته «{selectedCategory?.name}» هنوز ویژگی ندارد</strong><p className="mb-0 mt-1 text-xs leading-6 text-[var(--warning)]">ویژگی‌های هر محصول از دسته‌بندی آن خوانده می‌شوند. ابتدا ویژگی‌های موردنیاز این دسته را تعریف کنید.</p><Link href={`/admin/categories/${categoryId}/attributes`} className="mt-3 inline-flex min-h-10 items-center justify-center rounded-xl bg-white px-4 text-xs font-bold text-[var(--warning)] shadow-sm">تعریف ویژگی‌های این دسته</Link></div> : <div className="grid gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:p-4"><div className="grid gap-2"><p className="m-0 text-[11px] leading-5 text-slate-500">این محصول براساس دسته «{selectedCategory?.name}» دارای {attributeDefinitions.length.toLocaleString("fa-IR")} ویژگی قابل تکمیل است.</p><div className="grid grid-cols-3 gap-2">{[{ label: "تعریف‌شده", value: attributeDefinitions.length }, { label: "تکمیل‌شده", value: completedAttributeCount }, { label: "مهم", value: importantAttributeCount }].map((item) => <div key={item.label} className="rounded-lg border border-slate-200 bg-white px-3 py-2"><strong className="block text-base font-bold text-slate-800">{item.value.toLocaleString("fa-IR")}</strong><span className="text-[10px] text-slate-500">{item.label}</span></div>)}</div></div><div className="flex justify-end">{product ? categoryChanged ? <span className="rounded-lg bg-[color-mix(in_srgb,var(--warning)_14%,transparent)] px-3 py-2 text-[11px] font-bold text-[var(--warning)]">ابتدا تغییر دسته را ذخیره کنید</span> : <Link href={`/admin/products/${product.id}/attributes`} className="inline-flex min-h-10 items-center justify-center rounded-xl bg-violet-700 px-5 text-xs font-bold text-white">مدیریت ویژگی‌ها</Link> : <AdminSaveButton isSaving={loading} name="afterSave" value="attributes" label="ثبت و مدیریت ویژگی‌ها" savingLabel="در حال ثبت..." />}</div></div>}
        </FormSection>
