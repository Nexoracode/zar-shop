...
export function AppChrome({ header, footer, children, storefrontAvailable, maintenanceMode, storeName, brandStyle, compactMobileGrid }: { header: ReactNode; footer: ReactNode; children: ReactNode; storefrontAvailable: boolean; maintenanceMode: boolean; storeName: string; brandStyle: CSSProperties; compactMobileGrid: boolean }) {
  ...
  // Login/register/forgot-password are bare entry screens, not part of the storefront
  // browsing experience — they render standalone, same as /admin and /invoices.
  const isBareAuthPage = pathname === "/login" || pathname === "/register" || pathname === "/forgot-password";
  const isStandalone = pathname.startsWith("/admin") || pathname.startsWith("/invoices/") || isBareAuthPage;
