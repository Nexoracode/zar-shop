...
export function PaymentGatewayManager({ mode, initialConfigs }: { mode: "list" | "form"; initialConfigs: PublicGatewayConfig[] }) {
  ...
  if (mode === "list") return <Card variant="secondary" className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-sm" dir="rtl">
    ...
    {configs.length ? <>
      <div className="divide-y divide-[var(--border)] md:hidden">{configs.map((config) => <article key={config.id} className="p-4"><div className="flex items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-emerald-50 text-emerald-700"><CheckCircle2 size={18} /></span><div className="min-w-0 flex-1"><strong className="block truncate text-sm">{config.displayName}</strong><span className="font-mono text-xs text-[var(--muted)]" dir="ltr">{config.credentialMasked}</span></div><Chip size="sm" variant="soft" className={config.isSandbox ? "text-amber-700" : "text-emerald-700"}><Chip.Label>{config.isSandbox ? "آزمایشی" : "اصلی"}</Chip.Label></Chip></div><Button type="button" size="sm" variant="danger-soft" isPending={deleting === config.provider} onPress={() => void remove(config.provider)} className="mt-4 min-h-9 w-full gap-2"><Trash2 size={14} />حذف درگاه</Button></article>)}</div>
