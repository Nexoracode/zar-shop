...
export function PaymentGatewayManager({ mode, initialConfigs }: { mode: "list" | "form"; initialConfigs: PublicGatewayConfig[] }) {
  ...
  if (mode === "list") return <Card variant="secondary" className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-sm" dir="rtl">
    <div className="border-b border-[var(--border)] p-5"><h2 className="m-0 text-base font-bold">درگاه‌های ثبت‌شده</h2><p className="mb-0 mt-1 text-xs text-[var(--muted)]">برای تغییر شناسه، درگاه موردنظر را از صفحه افزودن دوباره ثبت کنید.</p></div>
    {configs.length ? <>
      <div className="divide-y divide-[var(--border)] md:hidden">{configs.map((config) => <article key={config.id} className="p-4"><div className="flex items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[color-mix(in_srgb,var(--success)_12%,transparent)] text-[var(--success)]"><CheckCircle2 size={18} /></span><div className="min-w-0 flex-1"><strong className="block truncate text-sm">{config.displayName}</strong><span className="font-mono text-xs text-[var(--muted)]" dir="ltr">{config.credentialMasked}</span></div><Chip size="sm" variant="soft" className={config.isSandbox ? "text-[var(--warning)]" : "text-[var(--success)]"}><Chip.Label>{config.isSandbox ? "آزمایشی" : "اصلی"}</Chip.Label></Chip></div><Button type="button" size="sm" variant="danger-soft" isPending={deleting === config.provider} onPress={() => void remove(config.provider)} className="mt-4 min-h-9 w-full gap-2"><Trash2 size={14} />حذف درگاه</Button></article>)}</div>
      <AdminBulkEditor entity="paymentGateways" entityLabel="درگاه" ids={configs.map((config) => config.id)} actions={[{ value: "delete", label: "حذف درگاه‌های انتخاب‌شده", confirmation: { title: "حذف گروهی درگاه‌ها", description: "اطلاعات اتصال رمزنگاری‌شده درگاه‌های انتخاب‌شده حذف می‌شود و پرداخت از طریق آن‌ها دیگر ممکن نخواهد بود.", confirmLabel: "حذف درگاه‌ها" } }]} onCompleted={({ ids }) => setConfigs((current) => current.filter((item) => !ids.includes(item.id)))}><Table><TableScrollContainer><TableContent aria-label="فهرست درگاه‌های پرداخت" className="w-full min-w-[680px]"><TableHeader><TableColumn id="select" className="w-12 bg-[var(--surface-secondary)] px-4 py-3 text-center"><span className="sr-only">انتخاب</span></TableColumn>{["درگاه", "شناسه اتصال", "محیط", "عملیات"].map((head, index) => <TableColumn id={head} key={head} isRowHeader={index === 0} className="bg-[var(--surface-secondary)] px-5 py-3 text-right text-[11px] font-bold text-[var(--muted)]">{head}</TableColumn>)}</TableHeader><TableBody>{configs.map((config) => <TableRow id={config.id} key={config.id} className="transition hover:bg-[var(--surface-secondary)]/70"><TableCell className="w-12 px-4 py-3 text-center"><AdminBulkCheckbox id={config.id} label={`انتخاب درگاه ${config.displayName}`} /></TableCell><TableCell className="w-56 max-w-56 px-5 py-3 align-middle"><span className="flex min-w-0 items-center gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-[color-mix(in_srgb,var(--success)_12%,transparent)] text-[var(--success)]"><CheckCircle2 size={17} /></span><TruncatedTextTooltip text={config.displayName} className="max-w-36 text-sm font-bold" /></span></TableCell><TableCell className="px-5 py-3 align-middle"><span className="block font-mono text-xs text-[var(--muted)]" dir="ltr">{config.credentialMasked}</span></TableCell><TableCell className="px-5 py-3 align-middle"><Chip size="sm" variant="soft" className={config.isSandbox ? "text-[var(--warning)]" : "text-[var(--success)]"}><Chip.Label>{config.isSandbox ? "آزمایشی" : "اصلی"}</Chip.Label></Chip></TableCell><TableCell className="px-5 py-3 align-middle"><Button type="button" isIconOnly size="sm" variant="danger-soft" isPending={deleting === config.provider} aria-label={`حذف ${config.displayName}`} onPress={() => void remove(config.provider)}><Trash2 size={15} /></Button></TableCell></TableRow>)}</TableBody></TableContent></TableScrollContainer></Table></AdminBulkEditor>
    </> : <div className="px-5 py-10 text-center text-xs text-[var(--muted)]">هنوز درگاهی ثبت نشده است.</div>}
  </Card>;
  ...
  return <div className="grid gap-5" dir="rtl">
    <Card variant="secondary" className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
      <div className="mb-4"><h2 className="m-0 text-base font-bold">درگاه‌های پیشنهادی</h2><p className="mb-0 mt-1 text-xs text-[var(--muted)]">درگاه موردنظر را انتخاب کنید تا راهنمای دریافت شناسه آن نمایش داده شود.</p></div>
    ...
    <form onSubmit={submit} className="grid items-start gap-5 lg:grid-cols-2">
      <Card variant="secondary" className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
        <div className="mb-4 flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-[var(--accent)]/10 text-[var(--accent)]"><ShieldCheck size={20} /></span><div><h2 className="m-0 text-base font-bold">فعال‌سازی {selected.name}</h2><p className="m-0 mt-1 text-xs text-[var(--muted)]">ابتدا شناسه اتصال را از پنل رسمی درگاه دریافت کنید.</p></div></div>
        <ol className="m-0 grid list-none gap-3 p-0">{selected.steps.map((step, index) => <li key={step} className="flex items-start gap-3 rounded-xl bg-[var(--surface-secondary)] p-3 text-xs leading-6"><span className="grid size-7 shrink-0 place-items-center rounded-lg bg-[var(--accent)]/10 font-bold text-[var(--accent)]">{(index + 1).toLocaleString("fa-IR")}</span><span>{step}</span></li>)}</ol>
