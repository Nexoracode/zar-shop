      ...
      <div className="grid min-w-0 gap-2.5 p-2.5 sm:p-3 md:grid-cols-2 2xl:grid-cols-3">
        {group.attributes.map((attribute) => {
          ...
          return <article key={attribute.id} className="min-w-0 rounded-lg border border-slate-200 bg-white p-2.5 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
            ...
            <div className="mt-2 flex min-w-0 items-center gap-1.5"><Input value={draft} onChange={(event) => setDrafts((current) => ({ ...current, [attribute.id]: event.target.value }))} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addMultipleValue(attribute.id); } }} aria-label={`مقدار ${attribute.name}`} placeholder="مقدار؛ چند مورد با کاما" fullWidth variant="secondary" className="!h-[30px] !min-h-[30px] min-w-0 !rounded-lg border border-[var(--field-border)] bg-[var(--field-background)] !px-2.5 !py-1 text-[11px] text-[var(--field-foreground)] outline-none transition placeholder:text-[var(--field-placeholder)] focus:border-[var(--field-border-focus)] focus:ring-2 focus:ring-[var(--focus)]/10" /><Button type="button" isIconOnly size="sm" variant="secondary" isDisabled={!draft.trim()} aria-label={`افزودن مقدار ${attribute.name}`} onPress={() => addMultipleValue(attribute.id)} className="!size-[30px] !min-h-[30px] !min-w-[30px] shrink-0 !rounded-lg !p-0"><Plus className="!size-3.5" /></Button></div>
