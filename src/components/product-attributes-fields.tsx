      ...
      <div className="grid min-w-0 gap-3 p-3 md:grid-cols-2">
        {group.attributes.map((attribute) => {
          ...
          return <article key={attribute.id} className="flex min-h-[150px] min-w-0 flex-col rounded-xl border border-slate-200 bg-white p-3.5 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
            <div className="min-w-0"><TruncatedTextTooltip text={attribute.name} className="max-w-full text-[13px] font-bold leading-5 text-slate-700" /></div>
            <div className="mt-2.5 flex min-w-0 items-center gap-2"><Input value={draft} maxLength={attributeFieldLimits.value} onChange={(event) => setDrafts((current) => ({ ...current, [attribute.id]: event.target.value }))} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addMultipleValue(attribute.id); } }} aria-label={`مقدار ${attribute.name}`} placeholder="مقدار؛ چند مورد با کاما" fullWidth variant="secondary" className="!h-[34px] !min-h-[34px] min-w-0 !rounded-lg border border-[var(--field-border)] bg-[var(--field-background)] !px-3 !py-1 !text-xs text-[var(--field-foreground)] outline-none transition placeholder:!text-[10px] placeholder:text-[var(--field-placeholder)] focus:border-[var(--field-border-focus)] focus:ring-2 focus:ring-[var(--focus)]/10" /><Button type="button" isIconOnly size="sm" variant="secondary" isDisabled={!draft.trim()} aria-label={`افزودن مقدار ${attribute.name}`} onPress={() => addMultipleValue(attribute.id)} className="!size-[34px] !min-h-[34px] !min-w-[34px] shrink-0 !rounded-lg !p-0"><Plus className="!size-4" /></Button></div>
