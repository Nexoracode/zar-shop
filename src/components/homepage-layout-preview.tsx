  ...
  return <div className="xl:sticky xl:top-5">
    ...
    <Card variant="secondary" className="overflow-hidden rounded-xl border border-[var(--border)] bg-[#e9eaed] p-2 shadow-inner">
      <div className={`mx-auto overflow-hidden rounded-lg border border-black/10 bg-[#f5f5f5] shadow-sm transition-all duration-300 ${mode === "mobile" ? "w-[220px]" : "w-full"}`}>
        <div className="flex h-5 items-center gap-1 border-b border-black/5 bg-white px-2"><span className="size-1.5 rounded-full bg-[var(--danger)]" /><span className="size-1.5 rounded-full bg-[var(--warning)]" /><span className="size-1.5 rounded-full bg-[var(--success)]" /><span className="mr-2 h-1.5 flex-1 rounded-full bg-slate-100" /></div>
...
function ProductShapes({ count = 4 }: { count?: number }) {
  return <div className="grid grid-cols-4 gap-1">{Array.from({ length: count }, (_, index) => <span key={index} className="grid gap-0.5"><i className="aspect-square rounded bg-slate-200" /><i className="h-1 rounded-full bg-slate-200" /><i className="h-1 w-2/3 rounded-full bg-[var(--warning)]" /></span>)}</div>;
}
...
function PreviewSection({ id, mode, heroUrl }: { id: HomepageSectionId; mode: PreviewMode; heroUrl: string | null }) {
  ...
  if (id === "FEATURED_PRODUCTS") return <PreviewBlock label={sectionNames[id]!} tone="bg-[color-mix(in_srgb,var(--danger)_12%,transparent)]"><ProductShapes count={4} /></PreviewBlock>;
  if (id === "BEST_SELLING_PRODUCTS") return <PreviewBlock label={sectionNames[id]!}><div className="grid grid-cols-4 gap-1">{Array.from({ length: 12 }, (_, index) => <span key={index} className="flex items-center gap-0.5"><i className="size-2.5 shrink-0 rounded bg-slate-200" /><i className="grid size-1.5 shrink-0 place-items-center rounded-full bg-[var(--danger)] text-[3px] text-white">{index + 1}</i><i className="h-1 flex-1 rounded-full bg-slate-200" /></span>)}</div></PreviewBlock>;
  if (id === "POPULAR_PRODUCTS" || id === "LATEST_PRODUCTS") return <PreviewBlock label={sectionNames[id]!}><ProductShapes count={4} /></PreviewBlock>;
  if (id === "ABOUT") return <PreviewBlock label={sectionNames[id]!}><div className="grid grid-cols-2 gap-1"><span className="h-10 rounded bg-[color-mix(in_srgb,var(--warning)_14%,transparent)]" /><span className="h-10 rounded bg-slate-200" /></div></PreviewBlock>;
