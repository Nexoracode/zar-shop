...
export function DeliveryAddressPicker({ initialAddresses, user, authenticated = true, compact = false }: { initialAddresses: StorefrontAddress[]; user: { firstName: string | null; lastName: string | null; phone: string | null }; authenticated?: boolean; compact?: boolean }) {
  ...
  const trigger = <Button type="button" variant="ghost" onPress={() => setOpen(true)} className={`h-auto min-h-0 gap-2 bg-transparent p-0 text-right hover:bg-transparent data-[hovered=true]:bg-transparent ${compact ? "max-w-64 text-[11px]" : "text-xs"}`}><MapPin size={compact ? 16 : 18} className="shrink-0 text-[var(--brand-primary)]" /><span className="min-w-0"><span className="block text-[10px] text-[var(--muted)]">ارسال به</span><strong className="block max-w-52 truncate text-[var(--foreground)]">{selected ? `${selected.city}، ${selected.addressLine}` : "انتخاب نشانی تحویل"}</strong></span><ChevronLeft size={14} className="shrink-0 text-[var(--muted)]" /></Button>;

  if (!authenticated) return <a href="/login?next=/account" className="flex items-center gap-2 text-[11px] text-[var(--muted)]"><MapPin size={17} />برای انتخاب نشانی وارد شوید</a>;

  return <>{trigger}<Modal.Backdrop isOpen={open} onOpenChange={(next) => { setOpen(next); if (!next) setEditing(null); }} variant="blur"><Modal.Container placement="center" size="lg"><Modal.Dialog aria-label="انتخاب نشانی تحویل" dir="rtl" className="mx-3 max-h-[calc(100dvh-24px)] overflow-hidden bg-[var(--surface)]"><Modal.Header className="flex-row items-center justify-between border-b border-[var(--border)] px-5 py-4"><div><Modal.Heading className="text-base font-black">{editing ? editing === "new" ? "افزودن نشانی جدید" : "ویرایش نشانی" : "انتخاب نشانی تحویل"}</Modal.Heading><p className="mb-0 mt-1 text-xs text-[var(--muted)]">{editing ? "اطلاعات پستی و گیرنده را کامل کنید." : "سفارش به نشانی انتخاب‌شده ارسال می‌شود."}</p></div><Modal.CloseTrigger aria-label="بستن" className="grid size-9 place-items-center rounded-lg text-[var(--muted)]"><X size={18} /></Modal.CloseTrigger></Modal.Header><Modal.Body className="overflow-y-auto p-5">{editing ? <AddressForm initial={editing === "new" ? null : editing} user={user} onSaved={replace} onCancel={() => setEditing(null)} /> : <div className="grid gap-3">{addresses.map((address) => <article key={address.id} className={`rounded-2xl border p-4 transition ${address.isDefault ? "border-[var(--brand-primary)] bg-[var(--brand-primary)]/5" : "border-[var(--border)]"}`}><div className="flex items-start gap-3"><Button type="button" variant="ghost" onPress={() => void select(address)} isPending={busy === address.id} className="h-auto min-w-0 flex-1 justify-start gap-3 bg-transparent p-0 text-right hover:bg-transparent data-[hovered=true]:bg-transparent"><span className={`mt-1 grid size-5 shrink-0 place-items-center rounded-full border ${address.isDefault ? "border-[var(--brand-primary)] bg-[var(--brand-primary)] text-[var(--brand-primary-foreground)]" : "border-[var(--border)]"}`}>{address.isDefault && <Check size={13} />}</span><span className="min-w-0"><strong className="block text-sm">{address.title}</strong><span className="mt-2 block text-xs leading-6 text-[var(--muted)]">{address.province}، {address.city}، {address.addressLine}، پلاک {address.plaque}</span><span className="mt-2 flex items-center gap-2 text-xs text-[var(--muted)]"><UserRound size={14} />{address.recipient}{address.recipientType === "OTHER" && <b className="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] text-amber-700">گیرنده دیگر</b>}</span></span></Button><div className="flex shrink-0 gap-1"><Button type="button" isIconOnly size="sm" variant="ghost" aria-label={`ویرایش ${address.title}`} onPress={() => setEditing(address)}><Pencil size={15} /></Button><Button type="button" isIconOnly size="sm" variant="danger-soft" aria-label={`حذف ${address.title}`} isDisabled={busy === address.id} onPress={() => void remove(address)}><Trash2 size={15} /></Button></div></div></article>)}{!addresses.length && <div className="grid min-h-48 place-items-center rounded-2xl border border-dashed border-[var(--border)] text-center"><div><MapPin className="mx-auto text-[var(--muted)]" size={34} /><strong className="mt-3 block text-sm">هنوز نشانی ثبت نکرده‌اید</strong><p className="mb-0 mt-1 text-xs text-[var(--muted)]">اولین نشانی به‌صورت پیش‌فرض انتخاب می‌شود.</p></div></div>}<Button type="button" variant="secondary" onPress={() => setEditing("new")} className="min-h-12 justify-start gap-2 rounded-xl border border-[var(--brand-primary)] px-4 font-bold text-[var(--brand-primary)]"><Plus size={18} />افزودن نشانی جدید</Button></div>}</Modal.Body></Modal.Dialog></Modal.Container></Modal.Backdrop></>;
}
