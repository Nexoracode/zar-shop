...
export function ArticleCard({ article }: { article: ArticleListItem }) {
  ...
  return (
    ...
    >
      <span className="relative block aspect-[16/9] w-full overflow-hidden bg-[var(--surface-secondary)] text-[var(--muted)]">
        {article.coverMedia ? (
          <Image src={article.coverMedia.url} alt={article.coverMedia.alt ?? article.title} fill sizes="(max-width: 768px) 100vw, 360px" className="object-cover transition group-hover:scale-[1.03]" />
        ) : (
          <span className="grid h-full place-items-center"><ImageOff size={28} /></span>
        )}
      </span>
