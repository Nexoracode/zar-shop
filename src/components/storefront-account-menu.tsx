    ...
    <Popover isOpen={isOpen} onOpenChange={setIsOpen}>
      ...
      <Popover.Content placement="bottom left" dir="rtl" className="z-[190] w-[min(92vw,290px)] rounded-xl border border-[var(--border)] bg-[var(--surface)] p-0 text-right text-[var(--foreground)] shadow-[0_10px_35px_rgba(15,23,42,.18)]">
        <Popover.Dialog dir="rtl" className="p-0 text-right">
          <Link href="/account/profile" onClick={() => setIsOpen(false)} className="flex min-h-16 items-center gap-3 border-b border-[var(--border)] px-5 font-black"><span className="min-w-0 flex-1 truncate text-sm">{name}</span><ChevronLeft size={18} className="text-[var(--muted)]" /></Link>
          <Link href="/account" onClick={() => setIsOpen(false)} className="flex min-h-12 items-center gap-3 border-b border-[var(--border)] px-5 text-xs font-bold text-[var(--brand-primary)]"><Sparkles size={18} /><span className="flex-1">باشگاه مشتریان</span><small className="font-normal text-[var(--muted)]">به‌زودی</small><ChevronLeft size={15} /></Link>
