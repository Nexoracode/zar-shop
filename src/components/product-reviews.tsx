...
export function ProductReviews({ productId, initialData, isAuthenticated }: Props) {
  ...
  return <div id="reviews" className="grid scroll-mt-24 gap-8 lg:grid-cols-[260px_minmax(0,1fr)]" dir="rtl">
    <aside className="h-fit rounded-xl border border-slate-200 p-5 lg:sticky lg:top-24">
      <div className="flex items-end gap-2"><strong className="text-2xl font-extrabold text-slate-900">{summary.average.toLocaleString("fa-IR", { maximumFractionDigits: 1 })}</strong><span className="pb-1 text-xs text-slate-400">از ۵</span></div>
      <div className="my-3 flex gap-1 text-[var(--warning)]" aria-label={`${summary.average.toLocaleString("fa-IR", { maximumFractionDigits: 1 })} از ۵ ستاره`}>{[1, 2, 3, 4, 5].map((star) => <Star key={star} size={18} fill={star <= Math.round(summary.average) ? "currentColor" : "none"} />)}</div>
      <p className="text-xs text-slate-500">از مجموع {summary.count.toLocaleString("fa-IR")} دیدگاه تأییدشده</p>
      <div className="mt-5 grid gap-2.5">{[5, 4, 3, 2, 1].map((score) => <div key={score} className="grid grid-cols-[28px_1fr] items-center gap-2 text-[10px] text-slate-500"><span>{score.toLocaleString("fa-IR")}</span><span className="h-1.5 overflow-hidden rounded-full bg-slate-100"><span className="block h-full rounded-full bg-[var(--warning)]" style={{ width: `${summary.count ? summary.distribution[score as 1 | 2 | 3 | 4 | 5] / summary.count * 100 : 0}%` }} /></span></div>)}</div>
...
function ReviewCard({ review, authenticated, busyVote, expandedReviewIds, onToggleExpanded, onReply, onVote, onReport }: {
  ...
  return <article className="relative py-6">
    <header className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-slate-100 text-sm font-bold text-slate-500">{review.author.name.slice(0, 1)}</span><div className="min-w-0 flex-1"><div className="flex min-h-10 items-center gap-2"><strong className="truncate text-sm font-bold text-slate-800">{review.author.name}</strong>{(review.author.isManagement || review.author.isVerifiedPurchase) && <span aria-hidden="true" className="text-slate-300">·</span>}{review.author.isManagement ? <span className="shrink-0 text-[10px] font-bold text-[var(--brand-primary)]">مدیریت فروشگاه</span> : review.author.isVerifiedPurchase ? <Chip size="sm" variant="soft" className="shrink-0 bg-[color-mix(in_srgb,var(--success)_12%,transparent)] text-[var(--success)]"><Chip.Label>خریدار</Chip.Label></Chip> : null}<div className="mr-auto flex shrink-0 items-center gap-1 text-[11px] text-slate-400"><span>{new Date(review.createdAt).toLocaleDateString("fa-IR")}</span>{!pending && !review.isOwn && <Button type="button" isIconOnly size="sm" variant="ghost" isDisabled={review.reportedByCurrentUser} aria-label={review.reportedByCurrentUser ? "گزارش شده" : "گزارش دیدگاه"} onPress={() => authenticated ? onReport(review) : toast.danger("برای گزارش دیدگاه وارد حساب کاربری شوید.")} className="size-8 min-h-8 min-w-8 text-slate-400"><MoreVertical size={17} /></Button>}</div></div>{review.rating && <div className="mt-1 flex items-center gap-2"><span className="flex items-center gap-0.5 text-[var(--warning)]" dir="ltr">{[1, 2, 3, 4, 5].map((star) => <Star key={star} size={14} fill={star <= review.rating! ? "currentColor" : "none"} />)}</span><span className="text-[11px] text-slate-500">{review.rating.toLocaleString("fa-IR")} از ۵</span></div>}</div></header>
