...
export function SmsConsentPreference({ initialValue }: { initialValue: boolean }) {
  ...
  return <Card variant="secondary" className="mt-3 rounded-2xl border border-[#e7e6e2] bg-white p-5" dir="rtl"><AdminCheckbox isSelected={value} isDisabled={saving} onChange={(next) => void update(next)} description="این گزینه فقط برای پیام‌های اطلاع‌رسانی و کمپین‌هاست؛ پیام‌های ضروری سفارش جداگانه مدیریت می‌شوند.">دریافت پیامک‌های فروشگاه</AdminCheckbox></Card>;
}
