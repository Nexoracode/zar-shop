...
import { Button, I18nProvider, Modal } from "@heroui/react";
 ...
 * edit, reorder and remove. It only validates: the entered links go into the page builder's draft (`onConfirm`) and are
 * saved with the rest of the page, so nothing is sent to the server from here.
 */
export function PageBuilderMenuDialog({ initialItems, linkOptions, onConfirm, onClose }: { initialItems: HomepageMenuItem[]; linkOptions: HomepageMenuLinkOption[]; onConfirm: (items: HomepageMenuItem[]) => void; onClose: () => void }) {
  ...
  function submit() {
    if (!validate()) return;
    onConfirm(items.map((item) => ({ id: item.id, label: item.label.trim(), href: item.href.trim() })));
  }
        ...
        <Modal.Dialog data-page-builder-ui aria-label="ویرایش منو" dir="rtl" className="p-0 mx-4 max-w-[640px] bg-[var(--surface)] text-right">
          ...
          <Modal.Body className="m-0 grid max-h-[60vh] gap-4 overflow-y-auto p-5">
            <div className="grid items-end gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
              <HeroSelectField name="builder-menu-ready-link" label="افزودن لینک آماده" ariaLabel="افزودن لینک آماده" placeholder="صفحه یا دسته‌بندی" value="" disabled={full} searchable options={linkOptions.map((option) => ({ value: option.id, label: `${option.group} — ${option.label}` }))} onValueChange={(id) => { const option = linkOptions.find((item) => item.id === id); if (option) addItem(option.label, option.href); }} />
              <Button type="button" variant="outline" isDisabled={full} onPress={() => addItem("", "")} className="min-h-10 gap-1.5 rounded-lg px-4 text-sm font-bold"><Plus size={16} />آیتم دلخواه</Button>
            </div>
              ...
              {items.map((item, index) => (
                <li key={item.id} className="grid gap-2 rounded-xl border border-[var(--border)] p-3 sm:grid-cols-[minmax(0,.8fr)_minmax(0,1.2fr)_auto] sm:items-start">
                  ...
                  <div className="flex items-center gap-1 sm:mt-6">
                    <Button type="button" isIconOnly variant="ghost" isDisabled={index === 0} aria-label={`انتقال ${item.label || "آیتم"} به بالا`} onPress={() => moveItem(index, -1)} className="size-9 min-h-9 min-w-9"><ArrowUp size={16} /></Button>
                    <Button type="button" isIconOnly variant="ghost" isDisabled={index === items.length - 1} aria-label={`انتقال ${item.label || "آیتم"} به پایین`} onPress={() => moveItem(index, 1)} className="size-9 min-h-9 min-w-9"><ArrowDown size={16} /></Button>
                    <Button type="button" isIconOnly variant="ghost" aria-label={`حذف ${item.label || "آیتم"}`} onPress={() => setItems((current) => current.filter((currentItem) => currentItem.id !== item.id))} className="size-9 min-h-9 min-w-9 text-[var(--danger)]"><Trash2 size={16} /></Button>
                  </div>
                </li>
              ))}
            </ul>
