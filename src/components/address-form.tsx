  ...
  return <form ref={formRef} onSubmit={submit} noValidate className="grid min-h-0" dir="rtl">
    <div className="grid gap-4 px-5 py-3 sm:px-6">
      <section className={step === 2 ? "grid gap-3" : "hidden"} aria-hidden={step !== 2}>
        ...
        <label className={labelClass}><span>آدرس<span className="mr-0.5 text-[var(--danger)]">*</span></span><TextArea name="addressLine" defaultValue={initial?.addressLine ?? ""} onChange={() => clearFieldError("addressLine")} aria-invalid={Boolean(fieldErrors.addressLine)} minLength={10} maxLength={1000} rows={1} fullWidth variant="secondary" className={`${fieldClass} ${fieldErrors.addressLine ? invalidFieldClass : ""}`} placeholder="مثال: خیابان، کوچه و جزئیات آدرس" />{fieldErrors.addressLine ? <span role="alert" className="-mt-1.5 text-[11px] font-normal leading-5 text-[var(--danger)]">{fieldErrors.addressLine}</span> : <span className="-mt-1.5 text-[11px] font-normal leading-5 text-slate-400">در صورت تغییر این بخش و ناهماهنگی آن با موقعیت مکانی، ممکن است ارسال سفارش با مشکل مواجه شود.</span>}</label>
        <div className="grid grid-cols-2 items-start gap-3"><label className={labelClass}><span>پلاک<span className="mr-0.5 text-[var(--danger)]">*</span></span><Input name="plaque" defaultValue={initial?.plaque ?? ""} onChange={() => clearFieldError("plaque")} aria-invalid={Boolean(fieldErrors.plaque)} fullWidth variant="secondary" className={`${inputFieldClass} ${fieldErrors.plaque ? invalidFieldClass : ""}`} />{fieldError("plaque")}</label><label className={labelClass}>واحد<Input name="unit" defaultValue={initial?.unit ?? ""} fullWidth variant="secondary" className={inputFieldClass} /><span aria-hidden="true" className="block min-h-4" /></label></div>
