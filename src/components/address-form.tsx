...
export function AddressForm({ initial, user, onSaved, onCancel }: { initial?: StorefrontAddress | null; user: { firstName: string | null; lastName: string | null; phone: string | null }; onSaved: (address: StorefrontAddress) => void; onCancel: () => void }) {
  ...
  return <form onSubmit={submit} className="grid gap-5" dir="rtl">
    ...
    <div className="grid gap-4 sm:grid-cols-2"><HeroSelectField name="provinceIdField" label="استان" value={provinceId} onValueChange={(value) => { setProvinceId(value); setCityId(""); setCities([]); setLocationsLoading(Boolean(value)); }} required disabled={locationsLoading && !provinces.length} options={provinces.map((item) => ({ value: item.id, label: item.name }))} /><HeroSelectField name="cityIdField" label="شهر" value={cityId} onValueChange={setCityId} required disabled={!provinceId || locationsLoading} options={cities.map((item) => ({ value: item.id, label: item.name }))} /></div>
