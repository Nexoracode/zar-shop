...
type AddressSummary = { addressLine: string; plaque: string; unit: string; postalCode: string };
...
export function AddressForm({ initial, user, onSaved, onCancel, onStepChange }: { initial?: StorefrontAddress | null; user: { firstName: string | null; lastName: string | null; phone: string | null }; onSaved: (address: StorefrontAddress) => void; onCancel: () => void; onStepChange?: (step: AddressFormStep) => void }) {
  ...
  const [summary, setSummary] = useState<AddressSummary>({ addressLine: initial?.addressLine ?? "", plaque: initial?.plaque ?? "", unit: initial?.unit ?? "", postalCode: initial?.postalCode ?? "" });
  ...
  function goToStep(nextStep: 2 | 3) {
    ...
    if (nextStep === 3) setSummary({ addressLine: String(form.get("addressLine") ?? ""), plaque: String(form.get("plaque") ?? ""), unit: String(form.get("unit") ?? ""), postalCode: String(form.get("postalCode") ?? "") });
    setStep(nextStep);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const nextRecipientErrors = validateAddressRecipient({ recipientType, recipient: recipientType === "SELF" ? selfName : String(form.get("recipient") ?? ""), phone: recipientType === "SELF" ? user.phone ?? "" : String(form.get("phone") ?? ""), recipientNationalId: String(form.get("recipientNationalId") ?? "") });
    ...
    const payload = {
      ...
      recipientNationalId: recipientType === "OTHER" ? form.get("recipientNationalId") : "",
  ...
  const provinceName = provinces.find((item) => item.id === provinceId)?.name ?? initial?.province ?? "";
  const cityName = cities.find((item) => item.id === cityId)?.name ?? initial?.city ?? "";
  ...
  return <form ref={formRef} onSubmit={submit} noValidate className="grid min-h-0" dir="rtl">
    <div className="grid gap-5 px-5 py-4 sm:px-6">
      ...
      <section className={step === 3 ? "grid gap-5" : "hidden"} aria-hidden={step !== 3}>
        <div className="relative grid h-24 place-items-center overflow-hidden rounded-lg border border-slate-200 bg-slate-100 [background-image:linear-gradient(30deg,transparent_40%,rgba(148,163,184,.28)_41%,rgba(148,163,184,.28)_43%,transparent_44%),linear-gradient(120deg,transparent_35%,rgba(148,163,184,.22)_36%,rgba(148,163,184,.22)_39%,transparent_40%)]"><span className="inline-flex items-center gap-2 rounded-full bg-[var(--brand-primary)] px-3 py-1 text-[11px] font-bold text-[var(--brand-primary-foreground)] shadow"><MapPin size={14} />محدوده آدرس انتخاب‌شده</span></div>
        <div className="flex items-start gap-3 rounded-lg border border-[var(--border)] p-4"><SlidersHorizontal size={20} className="mt-1 shrink-0 text-[var(--muted)]" /><p className="m-0 text-xs leading-7 text-slate-600">{provinceName}، {cityName}، {summary.addressLine}، پلاک {summary.plaque}{summary.unit ? `، واحد ${summary.unit}` : ""}<span className="block">کدپستی: <b dir="ltr">{summary.postalCode}</b></span></p></div>
        ...
        {recipientType === "SELF" ? <div className="rounded-lg bg-[var(--surface-secondary)] px-4 py-3 text-xs leading-6 text-[var(--muted)]">گیرنده: <b className="text-[var(--foreground)]">{selfName || "اطلاعات پروفایل ناقص"}</b>{user.phone && <span className="mr-2" dir="ltr">{user.phone}</span>}{(recipientErrors.recipient || recipientErrors.phone) && <span role="alert" className="mt-1 block text-[var(--danger)]">{recipientErrors.recipient ?? recipientErrors.phone}</span>}</div> : <div className="grid items-start gap-4 sm:grid-cols-2"><label className={labelClass}><span>نام و نام خانوادگی<span className="mr-0.5 text-[var(--danger)]">*</span></span><Input name="recipient" defaultValue={initial?.recipientType === "OTHER" ? initial.recipient : ""} onChange={() => clearRecipientError("recipient")} aria-invalid={Boolean(recipientErrors.recipient)} fullWidth variant="secondary" className={`${inputFieldClass} ${recipientErrors.recipient ? invalidFieldClass : ""}`} />{recipientError("recipient")}</label><label className={labelClass}><span>شماره همراه<span className="mr-0.5 text-[var(--danger)]">*</span></span><Input name="phone" defaultValue={initial?.recipientType === "OTHER" ? initial.phone : ""} onChange={() => clearRecipientError("phone")} aria-invalid={Boolean(recipientErrors.phone)} dir="ltr" inputMode="tel" fullWidth variant="secondary" className={`${inputFieldClass} ${recipientErrors.phone ? invalidFieldClass : ""}`} />{recipientError("phone")}</label><label className={`${labelClass} sm:col-span-2`}><span>کد ملی گیرنده<span className="mr-0.5 text-[var(--danger)]">*</span></span><Input name="recipientNationalId" defaultValue={initial?.recipientNationalId ?? ""} onChange={() => clearRecipientError("recipientNationalId")} aria-invalid={Boolean(recipientErrors.recipientNationalId)} dir="ltr" inputMode="numeric" minLength={10} maxLength={10} fullWidth variant="secondary" className={`${inputFieldClass} ${recipientErrors.recipientNationalId ? invalidFieldClass : ""}`} />{recipientError("recipientNationalId")}</label></div>}
      </section>

      {locationsLoading && step === 1 && <div className="flex items-center gap-2 text-xs text-[var(--muted)]"><Spinner size="sm" />در حال دریافت اطلاعات استان و شهر…</div>}
      {error && <Alert status="danger"><Alert.Description>{error}</Alert.Description></Alert>}
    </div>

    <div className="sticky bottom-0 z-10 flex gap-3 border-t border-[var(--border)] bg-white px-5 py-3 sm:px-6">{step < 3 ? <Button type="button" variant="primary" fullWidth isDisabled={locationsLoading} onPress={() => goToStep(step === 1 ? 2 : 3)} style={primaryButtonStyle} className="min-h-10 justify-center gap-2 rounded-lg px-6 text-sm font-black"><span>تأیید و ادامه</span><ChevronLeft size={16} /></Button> : <><Button type="button" variant="outline" fullWidth isDisabled={saving} onPress={onCancel} className="min-h-10 flex-1 rounded-lg border border-[var(--brand-primary)] bg-white text-sm font-bold text-[var(--brand-primary)]">انصراف</Button><Button type="submit" variant="primary" fullWidth isPending={saving} isDisabled={saving} style={primaryButtonStyle} className="min-h-10 flex-1 justify-center gap-2 rounded-lg px-5 text-sm font-black"><Check size={16} /><span>ذخیره آدرس</span></Button></>}</div>
  </form>;
}
