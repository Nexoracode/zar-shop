  ...
  return <form ref={formRef} onSubmit={submit} noValidate className="grid min-h-0" dir="rtl">
    <div className="grid gap-5 px-5 py-4 sm:px-6">
      <section className={step === 2 ? "grid gap-5" : "hidden"} aria-hidden={step !== 2}>
        ...
        <label className={labelClass}><span>آدرس<span className="mr-0.5 text-[var(--danger)]">*</span></span><TextArea name="addressLine" defaultValue={initial?.addressLine ?? ""} onChange={() => clearFieldError("addressLine")} aria-invalid={Boolean(fieldErrors.addressLine)} minLength={10} maxLength={1000} rows={2} fullWidth variant="secondary" className={`${fieldClass} ${fieldErrors.addressLine ? invalidFieldClass : ""}`} placeholder="مثال: خیابان، کوچه و جزئیات آدرس" />{fieldErrors.addressLine ? <span role="alert" className="-mt-2 text-[11px] font-normal leading-5 text-[var(--danger)]">{fieldErrors.addressLine}</span> : <span className="-mt-2 text-[11px] font-normal leading-5 text-slate-400">در صورت تغییر این بخش و ناهماهنگی آن با موقعیت مکانی، ممکن است ارسال سفارش با مشکل مواجه شود.</span>}</label>
