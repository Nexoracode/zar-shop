...
export function AddressForm({ initial, user, onSaved, onCancel }: { initial?: StorefrontAddress | null; user: { firstName: string | null; lastName: string | null; phone: string | null }; onSaved: (address: StorefrontAddress) => void; onCancel: () => void }) {
  ...
  return <form onSubmit={submit} noValidate className="grid gap-6 px-5 pt-5" dir="rtl">
    ...
    <div className="grid grid-cols-2 items-start gap-4"><label className={labelClass}><span>کدپستی<span className="mr-0.5 text-[var(--danger)]">*</span></span><Input name="postalCode" defaultValue={initial?.postalCode ?? ""} onChange={() => clearFieldError("postalCode")} aria-invalid={Boolean(fieldErrors.postalCode)} dir="ltr" inputMode="numeric" minLength={10} maxLength={10} fullWidth variant="secondary" className={`${inputFieldClass} ${fieldErrors.postalCode ? invalidFieldClass : ""}`} placeholder="باید ۱۰ رقمی باشد" />{fieldError("postalCode")}</label><label className={labelClass}>طبقه<Input name="floor" defaultValue={initial?.floor ?? ""} fullWidth variant="secondary" className={inputFieldClass} /><span aria-hidden="true" className="block min-h-4" /></label></div>
    <label className={labelClass}><span>عنوان آدرس<span className="mr-0.5 text-[var(--danger)]">*</span></span><Input name="title" defaultValue={initial?.title ?? "خانه"} onChange={() => clearFieldError("title")} aria-invalid={Boolean(fieldErrors.title)} fullWidth variant="secondary" className={`${inputFieldClass} ${fieldErrors.title ? invalidFieldClass : ""}`} placeholder="مثلاً خانه یا محل کار" />{fieldError("title")}</label>
