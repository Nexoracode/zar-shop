...
import { BadgePercent, Check, ChevronLeft, CreditCard, MapPin, PackageCheck, ShieldCheck, Wallet, X } from "lucide-react";
...
export function CheckoutForm({ settings, paymentMethods, currency, itemCount, initialQuote, initialAddresses, user, wallet }: { settings: CommerceSettings; paymentMethods: StorefrontPaymentMethod[]; currency: "IRR" | "IRT"; itemCount: number; initialQuote: Quote; initialAddresses: StorefrontAddress[]; user: { firstName: string | null; lastName: string | null; phone: string | null }; wallet: { balance: number; checkoutEnabled: boolean } }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    toast.success("سفارش با موفقیت ثبت شد", { description: paidFully ? "در حال انتقال به صفحهٔ فاکتور هستید." : "در حال انتقال امن به درگاه پرداخت هستید.", timeout: 4000 });
  ...
  return (
    <form ref={formRef} onSubmit={submit} className="grid min-w-0 items-start gap-4 lg:grid-cols-[minmax(0,1fr)_350px]" dir="rtl">
      <div className="grid min-w-0 gap-4">
        <Card variant="secondary" className="rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-sm">
          <Card.Content className="p-4 sm:p-5">
            ...
            <div className="mb-4 flex items-start justify-between gap-4"><div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-[var(--brand-primary)]/10 text-[var(--brand-primary)]"><MapPin size={18} /></span><div><h2 className="m-0 text-base font-bold">نشانی تحویل سفارش</h2><p className="mb-0 mt-1 text-xs text-[var(--muted)]">نشانی ارسال را پیش از پرداخت بررسی کنید.</p></div></div><DeliveryAddressPicker initialAddresses={initialAddresses} user={user} /></div>
            {selectedAddress ? <div className="rounded-lg border border-[var(--border)] bg-[var(--surface-secondary)]/45 px-3 py-2.5"><div className="flex flex-wrap items-center gap-2"><strong className="text-[13px]">{selectedAddress.title}</strong>{selectedAddress.isDefault && <span className="rounded-full bg-[var(--brand-primary)] px-2 py-0.5 text-[10px] font-bold text-[var(--brand-primary-foreground)]">پیش‌فرض</span>}</div><p className="mb-0 mt-1.5 text-[13px] leading-6">{selectedAddress.province}، {selectedAddress.city}، {selectedAddress.addressLine}، پلاک {selectedAddress.plaque}{selectedAddress.unit ? `، واحد ${selectedAddress.unit}` : ""}</p><div className="mt-1.5 text-[11px] text-[var(--muted)]">کد پستی: <b dir="ltr">{selectedAddress.postalCode}</b></div></div> : <Alert status="warning"><Alert.Description>برای ادامه، اولین نشانی تحویل خود را ثبت کنید.</Alert.Description></Alert>}
          </Card.Content>
        </Card>
