...
import { BadgePercent, Building2, Check, ChevronLeft, CreditCard, MapPin, PackageCheck, ShieldCheck, Truck, UserRound, UsersRound } from "lucide-react";
      ...
      <div className="grid gap-5">
        <Card variant="secondary" className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-sm">
          <Card.Content className="p-5 sm:p-6">
            ...
            <div className="mb-5 flex items-start justify-between gap-4"><div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-[var(--brand-primary)]/10 text-[var(--brand-primary)]"><MapPin size={20} /></span><div><h2 className="m-0 text-base font-black">نشانی تحویل سفارش</h2><p className="mb-0 mt-1 text-xs text-[var(--muted)]">نشانی ارسال را پیش از پرداخت بررسی کنید.</p></div></div><DeliveryAddressPicker initialAddresses={initialAddresses} user={user} /></div>
        ...
        <Card variant="secondary" className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-sm"><Card.Content className="p-5 sm:p-6"><div className="mb-5 flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-[var(--brand-primary)]/10 text-[var(--brand-primary)]"><UserRound size={20} /></span><div><h2 className="m-0 text-base font-black">تحویل‌گیرنده سفارش</h2><p className="mb-0 mt-1 text-xs text-[var(--muted)]">مشخص کنید سفارش به شما یا شخص دیگری تحویل داده می‌شود.</p></div></div><input type="hidden" name="recipientType" value={recipientType} /><div className="grid gap-3 sm:grid-cols-2"><Button type="button" variant="secondary" onPress={() => setRecipientType("SELF")} className={`h-auto min-h-20 justify-start gap-3 rounded-xl border p-4 text-right ${recipientType === "SELF" ? "border-[var(--brand-primary)] bg-[var(--brand-primary)]/5 ring-1 ring-[var(--brand-primary)]" : "border-[var(--border)]"}`}><UserRound size={20} /><span><strong className="block">خودم</strong><small className="mt-1 block font-normal text-[var(--muted)]">تحویل به صاحب حساب</small></span>{recipientType === "SELF" && <Check size={18} className="mr-auto text-[var(--brand-primary)]" />}</Button><Button type="button" variant="secondary" onPress={() => setRecipientType("OTHER")} className={`h-auto min-h-20 justify-start gap-3 rounded-xl border p-4 text-right ${recipientType === "OTHER" ? "border-[var(--brand-primary)] bg-[var(--brand-primary)]/5 ring-1 ring-[var(--brand-primary)]" : "border-[var(--border)]"}`}><UsersRound size={20} /><span><strong className="block">شخص دیگر</strong><small className="mt-1 block font-normal text-[var(--muted)]">تحویل سفارش به فردی دیگر</small></span>{recipientType === "OTHER" && <Check size={18} className="mr-auto text-[var(--brand-primary)]" />}</Button></div><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-xs font-medium text-[var(--muted)]">نام و نام خانوادگی گیرنده<Input key={`${recipientType}-name`} name="recipient" defaultValue={recipientType === "SELF" ? selfName : ""} required fullWidth variant="secondary" className={fieldClass} /></label><label className="grid gap-2 text-xs font-medium text-[var(--muted)]">شماره موبایل گیرنده<Input key={`${recipientType}-phone`} name="recipientPhone" defaultValue={recipientType === "SELF" ? user.phone ?? "" : ""} required dir="ltr" inputMode="tel" pattern="09[0-9]{9}" fullWidth variant="secondary" className={fieldClass} /></label>{recipientType === "OTHER" && <label className="grid gap-2 text-xs font-medium text-[var(--muted)] sm:col-span-2">کد ملی گیرنده<Input name="recipientNationalId" required dir="ltr" inputMode="numeric" minLength={10} maxLength={10} fullWidth variant="secondary" className={fieldClass} /></label>}</div></Card.Content></Card>
