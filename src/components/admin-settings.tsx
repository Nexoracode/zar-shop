...
export function GeneralSettings({ initialSettings }: { initialSettings: GeneralStoreSettingsInput }) {
  ...
  return <form onSubmit={submit} className="admin-sticky-save-form grid gap-5"><SettingsGrid>
    <SettingCard icon={<Store size={19} />} title="هویت فروشگاه" description="اطلاعات اصلی نمایش‌داده‌شده در سایت و فاکتور">
      <div className="grid gap-4 sm:grid-cols-2"><Field label="نام فروشگاه"><Input name="storeName" required maxLength={generalSettingsFieldLimits.storeName} defaultValue={initialSettings.storeName} variant="secondary" className={adminFieldClass} /></Field><Field label="شعار کوتاه"><Input name="tagline" required maxLength={generalSettingsFieldLimits.tagline} defaultValue={initialSettings.tagline} variant="secondary" className={adminFieldClass} /></Field></div>
      <Field label="توضیح کوتاه فروشگاه"><TextArea name="shortDescription" required maxLength={generalSettingsFieldLimits.shortDescription} defaultValue={initialSettings.shortDescription} rows={3} variant="secondary" className={adminFieldClass} /></Field>
    ...
    <SettingCard icon={<MapPin size={19} />} title="اطلاعات تماس و حقوقی" description="برای فوتر، فاکتور و صفحات اعتماد">
      <div className="grid gap-4 sm:grid-cols-2"><Field label="شماره تماس"><Input name="supportPhone" maxLength={generalSettingsFieldLimits.supportPhone} defaultValue={initialSettings.supportPhone ?? ""} dir="ltr" variant="secondary" className={adminFieldClass} /></Field><Field label="ایمیل پشتیبانی"><Input name="supportEmail" type="email" maxLength={generalSettingsFieldLimits.supportEmail} defaultValue={initialSettings.supportEmail ?? ""} dir="ltr" variant="secondary" className={adminFieldClass} /></Field></div>
      <Field label="نشانی فروشگاه"><TextArea name="storeAddress" maxLength={generalSettingsFieldLimits.storeAddress} defaultValue={initialSettings.storeAddress ?? ""} placeholder="نشانی کامل فروشگاه" rows={2} variant="secondary" className={adminFieldClass} /></Field>
      <div className="grid gap-4 sm:grid-cols-2"><Field label="شناسه ملی / کد اقتصادی"><Input name="legalIdentifier" maxLength={generalSettingsFieldLimits.legalIdentifier} defaultValue={initialSettings.legalIdentifier ?? ""} placeholder="برای فاکتور رسمی" variant="secondary" className={adminFieldClass} /></Field><Field label="ساعات پاسخ‌گویی"><Input name="supportHours" maxLength={generalSettingsFieldLimits.supportHours} defaultValue={initialSettings.supportHours ?? ""} variant="secondary" className={adminFieldClass} /></Field></div>
    </SettingCard>
  ...
  return <form onSubmit={submit} className="admin-sticky-save-form grid gap-5"><SettingsGrid>
    <SettingCard icon={<Clock3 size={19} />} title="انقضای سفارش‌های بدون اقدام" description="مانند فروشگاه‌های بزرگ، سفارش پرداخت‌نشده پس از مهلت تعیین‌شده منقضی می‌شود" className="lg:col-span-2" help={{ summary: "این قواعد چرخه سفارش پرداخت‌نشده و بازگرداندن منابع رزروشده را کنترل می‌کنند.", blocks: [{ title: "زمان‌بندی", items: ["مهلت انقضا مدت نهایی فرصت پرداخت مشتری است.", "زمان هشدار باید کمتر از مهلت انقضا باشد.", "مبدأ زمان مشخص می‌کند شمارش از ایجاد سفارش یا رویداد پیکربندی‌شده آغاز شود."] }, { title: "پس از انقضا", description: "بر اساس اکشن انتخاب‌شده، سفارش منقضی می‌شود و موجودی یا سهم پروموشن رزروشده قابل بازگردانی است." }, { title: "رقابت پرداخت و انقضا", tone: "important", description: "پرداخت تأییدشده در رقابت هم‌زمان با انقضا اولویت دارد. پردازش سفارش، پرداخت و پروموشن تراکنشی و idempotent است." }] }}>
      <div className="grid gap-5 lg:grid-cols-[minmax(240px,0.7fr)_minmax(0,1.3fr)]">
        <div className="grid content-start gap-3 rounded-2xl border border-[color-mix(in_srgb,var(--warning)_30%,transparent)] bg-[color-mix(in_srgb,var(--warning)_14%,transparent)] p-4 text-[var(--warning)]">
          <span className="grid size-11 place-items-center rounded-xl bg-[color-mix(in_srgb,var(--warning)_14%,transparent)] text-[var(--warning)]"><Clock3 size={21} /></span>
          <div><span className="text-xs text-[var(--warning)]">مهلت فعلی پرداخت</span><strong className="mt-1 block text-2xl font-bold">{settings.orderExpirationEnabled ? `${settings.orderExpirationMinutes.toLocaleString("fa-IR")} دقیقه` : "غیرفعال"}</strong></div>
          <p className="m-0 text-xs leading-6 text-[var(--warning)]">اگر مشتری در این زمان پرداخت را کامل نکند، سفارش مطابق اقدام انتخاب‌شده پردازش و ظرفیت پروموشن آزاد می‌شود.</p>
          <Chip size="sm" variant="soft" className="w-fit text-[var(--warning)]"><Chip.Label>فقط سفارش‌های در انتظار پرداخت</Chip.Label></Chip>
        </div>
...
export function CommerceSettings({ initialSettings, configuredGatewayCount }: { initialSettings: CommerceSettingsData; configuredGatewayCount: number }) {
  ...
  return <form onSubmit={submit} className="admin-sticky-save-form grid gap-5"><div className="grid items-start gap-5 lg:grid-cols-[minmax(280px,0.82fr)_minmax(0,1.18fr)]">
    <SettingCard icon={<CreditCard size={19} />} title="روش‌های پرداخت" description="درگاه‌ها و ترتیب نمایش در تسویه حساب" help={{ summary: "فعال‌بودن پرداخت آنلاین و دسترسی مشتری به درگاه‌های ثبت‌شده از اینجا کنترل می‌شود.", blocks: [{ title: "پیش‌نیاز", description: "ابتدا حداقل یک درگاه را در صفحه مدیریت درگاه‌ها با شناسه معتبر ثبت کنید، سپس پرداخت آنلاین را فعال کنید." }, { title: "محیط آزمایشی", tone: "important", description: "قبل از انتشار، مطمئن شوید درگاه اصلی فعال است و تنظیم sandbox فقط در محیط توسعه روشن مانده است." }] }}>
      ...
      <div className={`flex items-center justify-between gap-3 rounded-xl border p-3 ${configuredGatewayCount ? "border-[color-mix(in_srgb,var(--success)_30%,transparent)] bg-[color-mix(in_srgb,var(--success)_12%,transparent)] text-[var(--success)]" : "border-[color-mix(in_srgb,var(--warning)_30%,transparent)] bg-[color-mix(in_srgb,var(--warning)_14%,transparent)] text-[var(--warning)]"}`}><span className="flex items-center gap-2 text-sm font-bold">{configuredGatewayCount ? <CheckCircle2 size={17} /> : <CreditCard size={17} />}درگاه‌های ثبت‌شده</span><Chip size="sm" variant="soft"><Chip.Label>{configuredGatewayCount.toLocaleString("fa-IR")} درگاه</Chip.Label></Chip></div>
    ...
    <SettingsGrid>
      <SettingCard icon={<FileQuestion size={19} />} title="سوالات متداول" description="برای تغییر ترتیب، هر سوال را از دستگیره جابه‌جا کنید" className="lg:col-span-2" help={{ summary: "سؤال‌ها، پاسخ‌ها، وضعیت انتشار و ترتیب نمایش FAQ در همین کارت مدیریت می‌شوند.", blocks: [{ title: "مدیریت سؤال", items: ["سؤال و پاسخ روشن و کوتاه بنویسید.", "برای پنهان‌کردن موقت، وضعیت انتشار را خاموش کنید.", "برای تغییر ترتیب، دستگیره سؤال را گرفته و در جای جدید رها کنید."] }, { title: "حذف", tone: "important", description: "حذف سؤال پس از ذخیره نهایی می‌شود. برای نگهداری سابقه، در صورت تردید آن را غیرفعال کنید." }] }}>
        <div className="grid gap-3">{faqs.map((faq, index) => <div key={faq.id} onDragOver={(event) => event.preventDefault()} onDrop={() => dropFaq(faq.id)} className={`grid gap-3 rounded-xl border bg-[var(--surface-secondary)] p-3 transition sm:grid-cols-[auto_auto_minmax(0,1fr)_auto] ${draggedFaqId === faq.id ? "border-[var(--accent)] opacity-50" : "border-[var(--border)]"}`}>
          ...
          <span className="mt-1 grid size-8 shrink-0 place-items-center rounded-lg bg-[var(--surface)] text-[11px] font-bold">{(index + 1).toLocaleString("fa-IR")}</span>
          ...
          <div className="mt-6 flex items-start gap-1 sm:flex-col"><Button type="button" isIconOnly size="sm" variant="ghost" aria-label={`${faq.enabled ? "غیرفعال‌کردن" : "فعال‌کردن"} سوال`} onPress={() => updateFaq(faq.id, { enabled: !faq.enabled })}>{faq.enabled ? <Eye size={16} className="text-[var(--success)]" /> : <EyeOff size={16} className="text-[var(--muted)]" />}</Button><Button type="button" isIconOnly size="sm" variant="danger-soft" aria-label="حذف سوال" onPress={() => setFaqs((current) => current.filter((item) => item.id !== faq.id))}><Trash2 size={15} /></Button></div>
        </div>)}</div>
