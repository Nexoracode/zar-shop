    ...
    <SettingCard icon={<LayoutTemplate size={19} />} title="قالب پنل مدیریت" description="ظاهر کلی پنل مدیریت را انتخاب کنید" className="lg:col-span-2" help={{ summary: "قالب انتخاب‌شده فقط ظاهر پنل مدیریت را تغییر می‌دهد و روی طراحی فروشگاه اثری ندارد.", blocks: [{ title: "کلاسیک", description: "ظاهر پیش‌فرض فعلی پنل مدیریت با گوشه‌های گرد و سایه‌های نرم." }, { title: "صنعتی", description: "قالب دوم با گوشه‌های تیز، حاشیه‌های ظریف و تایپوگرافی متراکم؛ حالت تاریک ندارد." }] }}>
      <Tabs selectedKey={adminTemplate} onSelectionChange={(key) => setAdminTemplate(String(key) as "CLASSIC" | "INDUSTRY")} className="w-full">
        <Tabs.List aria-label="انتخاب قالب پنل مدیریت" className="grid w-full grid-cols-2 gap-2 bg-transparent p-0">
          ...
          <Tabs.Tab id="INDUSTRY" className="min-h-11 min-w-0 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-4 text-xs font-bold outline-none transition hover:bg-[var(--surface-secondary)] data-[selected]:border-[var(--accent)] data-[selected]:bg-[var(--accent)]/8 data-[selected]:text-[var(--accent)]">صنعتی (طرح دوم)</Tabs.Tab>
        </Tabs.List>
      </Tabs>
