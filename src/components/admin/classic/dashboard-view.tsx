...
export function ClassicDashboardView({ isFullAdmin, activeProducts, customers, actionableOrders, revenueTotal, lowStockThreshold, recentOrders, lowStockProducts }: AdminDashboardData) {
  const stats = [
    {
    ...
    <>
      ...
      <div className={`mt-6 grid gap-6 ${isFullAdmin ? "xl:grid-cols-[minmax(0,1.55fr)_minmax(280px,0.75fr)]" : "grid-cols-1"}`}>
            ...
            <>
              <AdminBulkEditor entity="orders" entityLabel="سفارش" ids={recentOrders.map((order) => order.id)} actions={[{ value: "status:PROCESSING", label: "شروع آماده‌سازی سفارش‌های پرداخت‌شده" }, { value: "status:SHIPPED", label: "ثبت ارسال سفارش‌های در حال آماده‌سازی" }, { value: "status:DELIVERED", label: "ثبت تحویل سفارش‌های ارسال‌شده" }, { value: "status:CANCELLED", label: "لغو سفارش‌های پرداخت‌نشده" }]}><Table><TableScrollContainer><TableContent aria-label="آخرین سفارش‌ها" className="w-full min-w-[680px]"><TableHeader><TableColumn id="select" className="w-12 bg-slate-50/70 px-3 py-3 text-center"><span className="sr-only">انتخاب</span></TableColumn>{["شماره سفارش", "مشتری", "اقلام", "مبلغ", "وضعیت", "تاریخ"].map((title, index) => <TableColumn id={title} key={title} isRowHeader={index === 0} className="bg-slate-50/70 px-4 py-3 text-right text-[0.7rem] font-bold text-slate-500">{title}</TableColumn>)}</TableHeader><TableBody>{recentOrders.map((order) => (
                    <TableRow id={order.id} key={order.id} className="transition hover:bg-slate-50/60">
                      ...
                      <TableCell className="w-44 max-w-44 px-4 py-3"><TruncatedTextTooltip text={order.customerName} className="max-w-36 text-xs text-slate-600" /></TableCell>
                      <TableCell className="px-4 py-3 text-xs text-slate-500">{order.itemCount.toLocaleString("fa-IR")}</TableCell>
                      <TableCell className="px-4 py-3 text-xs font-bold text-slate-700">{formatMoney(order.total)}</TableCell>
              ...
              <div className="divide-y divide-slate-100 md:hidden">
                {recentOrders.map((order) => (
                  <article key={order.id} className="grid gap-3 p-4">
                    <div className="flex items-start justify-between gap-3"><div className="min-w-0"><strong className="block text-xs text-slate-700" dir="ltr">{order.orderNumber}</strong><span className="block truncate text-xs text-slate-400">{order.customerName}</span></div><AdminStatusBadge tone={orderStatusTones[order.status]}>{orderStatusLabels[order.status]}</AdminStatusBadge></div>
                    <div className="flex items-center justify-between gap-3 text-xs"><span className="font-bold text-slate-700">{formatMoney(order.total)}</span><span className="text-slate-400">{order.itemCount.toLocaleString("fa-IR")} قلم · {formatDate(order.createdAt)}</span></div>
                  </article>
                ))}
              </div>
            </>
        ...
        {isFullAdmin ? <div className="grid content-start gap-6">
          <AdminPanel>
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-4 sm:px-5"><div className="flex items-center gap-2"><span className="grid h-9 w-9 place-items-center rounded-xl bg-rose-50 text-rose-600"><TriangleAlert size={18} /></span><div><h2 className="m-0 text-sm font-bold text-slate-800">هشدار موجودی</h2><p className="m-0 text-[0.68rem] text-slate-400">محصولات با موجودی {lowStockThreshold.toLocaleString("fa-IR")} عدد یا کمتر</p></div></div></div>
