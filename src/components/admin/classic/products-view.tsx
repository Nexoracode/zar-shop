...
import { AdminBulkCheckbox, AdminBulkEditor, AdminBulkRowArea } from "@/components/admin-bulk-editor";
...
function ProductTableRow({ product, rowNumber, lowStockThreshold }: { product: ProductRow; rowNumber: number; lowStockThreshold: number }) {
  ...
  return <TableRow id={product.id} className="transition hover:bg-slate-50/70"><TableCell className={`${cell} w-12 text-center`}><AdminBulkCheckbox id={product.id} label={`انتخاب محصول ${product.name}`} /></TableCell><TableCell className={`${cell} w-16 font-bold text-slate-400`}><AdminBulkRowArea id={product.id}>{rowNumber.toLocaleString("fa-IR")}</AdminBulkRowArea></TableCell><TableCell className={`${cell} w-[340px] max-w-[340px]`}><AdminBulkRowArea id={product.id}><div className="flex min-w-0 items-center gap-3"><ProductThumb product={product} /><div className="min-w-0 flex-1"><div className="flex min-w-0 items-center gap-2"><TruncatedTextTooltip text={product.name} className="max-w-[180px] font-bold text-slate-800" />{product.featured && <Star size={14} className="shrink-0 fill-[var(--warning)] text-[var(--warning)]" />}{isProductDiscountActive(product) && <span title={discountTooltip(product)} className="inline-flex shrink-0 cursor-help items-center gap-1 rounded-lg bg-rose-50 px-2 py-1 text-[10px] font-bold text-rose-600"><BadgePercent size={13} />تخفیف فعال</span>}{product._count.variants > 0 && <span className="inline-flex shrink-0 cursor-help items-center gap-1 rounded-lg bg-violet-50 px-2 py-1 text-[10px] font-bold text-violet-700" title={variantTooltip(product)}><ListTree size={13} />دارای تنوع</span>}</div><TruncatedTextTooltip text={product.category?.name ?? "بدون دسته‌بندی"} className="max-w-[220px] text-xs text-slate-400" /></div></div></AdminBulkRowArea></TableCell><TableCell className={`${cell} text-right font-mono text-xs text-slate-500`}><AdminBulkRowArea id={product.id}>{product.sku}</AdminBulkRowArea></TableCell><TableCell className={`${cell} text-slate-600`}><AdminBulkRowArea id={product.id}>{product.storeIndustry === "GOLD" ? `${Number(product.weightGrams).toLocaleString("fa-IR", { maximumFractionDigits: 3 })} گرم` : product.fixedPrice ? formatMoney(product.fixedPrice.toString()) : "بدون قیمت"}</AdminBulkRowArea></TableCell><TableCell className={cell}><AdminBulkRowArea id={product.id}><span className={product.stock <= lowStockThreshold ? "font-bold text-rose-600" : "text-slate-600"}>{product.stock.toLocaleString("fa-IR")}</span></AdminBulkRowArea></TableCell><TableCell className={cell}><AdminBulkRowArea id={product.id}><AdminStatusBadge tone={productStatusTones[product.status]}>{productStatusLabels[product.status]}</AdminStatusBadge></AdminBulkRowArea></TableCell><TableCell className={cell}><div className="flex items-center gap-1"><Link href={`/admin/products/${product.id}/edit`} aria-label={`ویرایش محصول ${product.name}`} title="ویرایش محصول" className="inline-flex h-9 min-h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-[var(--accent)] transition hover:border-[var(--warning)] hover:text-[var(--warning)]"><Pencil size={15} /></Link><ProductDeleteButton id={product.id} name={product.name} disabled={product._count.orderItems > 0} iconOnly /></div></TableCell></TableRow>;
}
