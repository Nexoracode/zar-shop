...
export type ProductRow = Prisma.ProductGetPayload<{ include: { category: true; brand: { select: { name: true } }; media: { include: { media: true } }; _count: { select: { variants: true; orderItems: true } }; optionTypes: { select: { type: { select: { name: true } }; values: { select: { value: { select: { label: true; color: { select: { hex: true } } } } } } } } } }>;
