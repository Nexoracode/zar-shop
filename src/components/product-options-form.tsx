    ...
    <Card variant="secondary" className="rounded-xl border border-slate-200/80 bg-white shadow-sm">
      <Card.Content className="p-4 sm:p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
          <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#fbf7ef] text-[#9a7434]"><ListPlus size={19} /></span><div><h2 className="text-base font-bold text-slate-800">گروه‌های تنوع</h2><p className="text-xs text-slate-400">برای هر گروه، عنوان و مقادیر قابل انتخاب مشتری را مشخص کنید.</p></div></div>
        ...
        <div className="grid gap-3">
          {options.map((option, index) => <div key={option.key} onDragOver={(event) => event.preventDefault()} onDrop={() => { moveOption(option.key); setDraggedKey(null); }} className={`rounded-xl border bg-slate-50/40 p-3 sm:p-4 ${draggedKey === option.key ? "border-[#b5904c] opacity-50" : "border-slate-200"}`}>
            <div className="mb-3 flex items-center justify-between gap-2"><span className="rounded-md bg-[#f4ead8] px-2 py-1 text-[11px] font-bold text-[#785b27]">تنوع {(index + 1).toLocaleString("fa-IR")}</span><span className="text-[11px] text-slate-400">{option.values.length.toLocaleString("fa-IR")} مقدار</span></div>
