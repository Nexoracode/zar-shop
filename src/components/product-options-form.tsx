        ...
        <div className="grid gap-3">
          {options.map((option, index) => <div key={option.key} onDragOver={(event) => event.preventDefault()} onDrop={() => { moveOption(option.key); setDraggedKey(null); }} className={`rounded-xl border bg-white p-3 shadow-sm ${draggedKey === option.key ? "border-[#b5904c] opacity-50" : "border-slate-200"}`}>
            ...
            {option.values.length ? <div className="mt-4 grid gap-1.5 overflow-x-auto pb-1">
              <div className="grid min-w-[440px] grid-cols-[minmax(100px,1fr)_88px_190px_34px] gap-2 px-3 text-[10px] font-bold text-slate-400"><span>مقدار</span><span>موجودی</span><span>{storeIndustry === "GOLD" ? "وزن (گرم)" : "قیمت (ریال)"}</span><span className="sr-only">وضعیت</span></div>
              {option.values.map((item) => {
                ...
                return <div key={item.value} className={`grid min-w-[440px] grid-cols-[minmax(100px,1fr)_88px_190px_34px] items-start gap-2 rounded-lg border px-3 py-2 ${item.isActive ? "border-slate-200 bg-slate-50/60" : "border-slate-200 bg-slate-50 opacity-55"}`}>
