      ...
      <Card.Content className="p-4 sm:p-6">
        ...
        <div className="grid gap-3">
          {options.map((option, index) => <div key={option.key} onDragOver={(event) => event.preventDefault()} onDrop={() => { moveOption(option.key); setDraggedKey(null); }} className={`rounded-xl border bg-white p-3 shadow-sm ${draggedKey === option.key ? "border-[#b5904c] opacity-50" : "border-slate-200"}`}>
            ...
            {option.values.length ? <div className="mt-3 grid gap-1.5">{option.values.map((item) => { const color = colors.find((candidate) => candidate.id === item.colorId); return <div key={item.value} className={`grid grid-cols-[minmax(90px,1fr)_90px_minmax(130px,180px)_34px] items-end gap-2 rounded-lg border px-2.5 py-2 ${item.isActive ? "border-slate-200 bg-white" : "border-slate-200 bg-slate-50 opacity-55"}`}><span className="mb-2 inline-flex min-w-0 items-center gap-2 truncate text-xs font-bold text-slate-700">{color && <span className="h-4 w-4 shrink-0 rounded-full border border-slate-300" style={{ backgroundColor: color.hex }} />}{color?.name ?? item.value}</span><label className="grid gap-1 text-[10px] font-bold text-slate-400">موجودی<Input type="number" min="0" value={String(item.stock ?? 0)} onChange={(event) => updateOption(option.key, { values: option.values.map((value) => value.value === item.value ? { ...value, stock: Math.max(0, Number(event.target.value) || 0) } : value) })} variant="secondary" className="h-9 min-h-9 px-2 text-xs" aria-label={`موجودی ${item.value}`} /></label><label className="grid gap-1 text-[10px] font-bold text-slate-400">افزایش قیمت (ریال)<Input type="number" min="0" step="1" value={String(item.priceAdjustment)} onChange={(event) => updateOption(option.key, { values: option.values.map((value) => value.value === item.value ? { ...value, priceAdjustment: Math.max(0, Math.trunc(Number(event.target.value) || 0)) } : value) })} variant="secondary" className="h-9 min-h-9 px-2 text-xs" aria-label={`افزایش قیمت ${item.value}`} /></label>{item.persisted ? <Button type="button" size="sm" isIconOnly variant="ghost" aria-label={`${item.isActive ? "غیرفعال‌سازی" : "فعال‌سازی"} ${item.value}`} onPress={() => updateOption(option.key, { values: option.values.map((value) => value.value === item.value ? { ...value, isActive: !value.isActive } : value) })} className={`mb-0.5 h-8 min-h-8 w-8 min-w-8 ${item.isActive ? "text-emerald-600" : "text-slate-400"}`}>{item.isActive ? <Eye size={15} /> : <EyeOff size={15} />}</Button> : <Button type="button" size="sm" isIconOnly variant="danger-soft" onPress={() => updateOption(option.key, { values: option.values.filter((value) => value.value !== item.value) })} className="mb-0.5 h-8 min-h-8 w-8 min-w-8" aria-label={`حذف مقدار ${item.value}`}><X size={13} /></Button>}</div>; })}</div> : <p className="mt-2 text-xs text-amber-700">حداقل یک مقدار به این تنوع اضافه کنید.</p>}
          </div>)}
